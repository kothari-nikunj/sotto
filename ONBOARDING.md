...
| **Telegram** — a bot token from @BotFather, no phone pairing. Right call when WhatsApp pairing is painful. ~5 min, two variables (`SOTTO_CRON_DELIVER=telegram`). Less tested than WhatsApp. | | **Anthropic / OpenAI / Kimi / DeepSeek** — for the **chat** layer only (Ask Sotto, nudge replies). The briefs still need a Gemini key. |
...
2. **Variables** — add four:
   ...
   *(Delivering to Telegram instead? Those two `WHATSAPP_*` variables become your bot's variables
   plus `SOTTO_CRON_DELIVER=telegram` and `WHATSAPP_ENABLED=false` — [CHANNELS.md](CHANNELS.md) has
   the five steps.)*
...
*On Telegram or iMessage instead? Skip this tile — it stays grey and does **not** block the wizard
from completing. Your channel is set by variables ([CHANNELS.md](CHANNELS.md)), not here.*
