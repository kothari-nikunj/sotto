...
**Option A — one-click (recommended).** Click this: Railway builds the container, adds the `/data`
...
- **Proactive nudges** — a mostly-silent watcher every ~15 min; it only pings you for something
  time-sensitive (a meeting about to start, a due commitment, a birthday) with a draft ready.
- **Follow-up drafts** — 4:45pm, for meetings that ended that day; silent when there's nothing to send.
...
`SOTTO_PROACTIVE*` / `SOTTO_FOLLOWUP*` variables in [RAILWAY.md](RAILWAY.md).
...
**Staying current:** Sotto's skills update themselves on every redeploy; the Bridge updates by
downloading the newest DMG from Releases (pairing + permissions persist). The full picture — including
upgrading the underlying Hermes runtime — is **RAILWAY.md § Keeping Sotto up to date**.
