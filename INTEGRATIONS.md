...
- Keys only, no `/setup` tile: a key tile would need a route, an on-volume secret store, and a way
  to hand that secret to cron-launched subprocesses that read the environment — a new mechanism for
  two services. Environment variables already do this, so these two stay environment variables.
