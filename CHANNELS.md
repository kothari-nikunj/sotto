...
```bash
hermes gateway setup        # choose WhatsApp
```
It shows a QR code; open WhatsApp ▸ Settings ▸ Linked Devices ▸ Link a Device, scan it. Done — the brief
now arrives as a WhatsApp message from your linked number. No Mac involvement, no token.
