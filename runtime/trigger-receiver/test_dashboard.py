...
def test_post_company_about_rides_the_same_endpoint_and_cli(tmp_path):
    """A company has no facts map, so its correction lane is the About paragraph — same endpoint,
    same knowledge_edit.py, same audit rail as a person's fact correction."""
    m, srv, base = _server(tmp_path)
    _fixtures(str(tmp_path))
    rec = _stub_cli(m, tmp_path)
    try:
        _, authed = _login_with_csrf(base)
        code, body, _ = _post_json(base, "/api/people/acme-corp/facts",
                                   {"op": "company-about",
                                    "text": "Acme forges anvils for enterprise coyotes."},
                                   headers=authed)
        assert code == 200
        assert _cli_calls(rec)[0] == {"slug": "acme-corp", "op": "company-about",
                                      "text": "Acme forges anvils for enterprise coyotes."}
        writes = _audit_writes(tmp_path)
        assert writes[0]["op"] == "company-about" and writes[0]["target"] == "acme-corp"
        # text is validated exactly like a fact correction's
        assert _post_json(base, "/api/people/acme-corp/facts",
                          {"op": "company-about", "text": "   "}, headers=authed)[0] == 400
    finally:
        srv.shutdown()
