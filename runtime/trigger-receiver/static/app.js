...
(function () {
  ...
  function splitDocketEvents(cal) {
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      if (!attendeeNames(ev, out.selfEmail).length) continue;
  ...
  function meetingRow(ev, selfEmail, researchByEmail) {
    ...
    // Research lives WITH its meeting: anything known about this event's attendees
    // renders right under the row, not in a separate section.
    if (researchByEmail) {
      var att = Array.isArray(ev.attendees) ? ev.attendees : [];
      var shown = {};
      for (var i = 0; i < att.length; i++) {
        var em = att[i] && typeof att[i] === "object" ? str(att[i].email).toLowerCase() : "";
        var entry = em && !shown[em] ? researchByEmail[em] : null;
        if (!entry) continue;
        shown[em] = true;
        appendResearchLines(mainCol, entry, /*withName=*/ Object.keys(shown).length > 1 || att.length > 2);
      }
    }
    ...
    // commute-type events) is still just a place.
    if (isUrl(link)) {
      var a = el("a", "go", "join →");
      a.href = link;
      a.target = "_blank";
  ...
  function buildDocket(cal, research, googleOk) {
    ...
    if (!cal) {
      ...
      for (i = 0; i < todays.length; i++) {
        ledger.appendChild(meetingRow(todays[i], selfEmail, researchByEmail));
      }
      if (tomorrows.length) {
  ...
  /* The lines worth knowing about one attendee, appended under their meeting:
     who they are (title @ company or the company's one-liner), what's new (Recently),
     and the opener (Hook). withName prefixes the person's name when a meeting has
     several researched attendees, so lines don't blur together. */
  function appendResearchLines(mainCol, entry, withName) {
    var whoBits = [];
    ...
    if (who) {
      ...
      if (withName && name) whoLine.appendChild(el("span", "mini-label", name));
      whoLine.appendChild(el("span", "research-who", who));
  ...
  function personRow(p) {
    ...
    if (p.type === "company") {
      // Companies carry prose, not facts — "0 facts" would be a lie about a full page.
      folio.appendChild(el("span", null, "company"));
      if (upd) folio.appendChild(el("span", null, "updated " + upd));
    } else {
  ...
  /* Company dossier body: the server's `sections` ([{heading, text}], parsed from
     ...
     section labels. The first cap's folio carries the updated date — companies
     have no fact count worth boasting about. */
  function renderCompanySections(frag, data) {
    var sections = Array.isArray(data.sections) ? data.sections : [];
    var updShort = data.updated_at ? shortDate(data.updated_at) : "";
    var capRight = updShort && updShort !== String(data.updated_at)
      ? capCount("updated " + updShort) : null;
    if (!sections.length) {
      frag.appendChild(ledgerCap("On file", capRight));
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      frag.appendChild(ledgerCap(heading, i === 0 ? capRight : null));
  ...
  function paintPrefs(seq, container, prefs) {
    ...
    // No rule under this cap: each rule group below opens with its own ledger
    // top rule, and stacking a section rule on those reads as line clutter.
  ...
  function recordRow(entry) {
    ...
    if (entry.source === "outcome") {
      ...
      if (bits.length) folio.appendChild(el("span", null, bits.join(" · ")));
    } else if (entry.source === "triage" && str(entry.channel)) {
      folio.appendChild(el("span", null, humanChannel(entry.channel)));
    }
    row.appendChild(folio);
    return row;
  }
