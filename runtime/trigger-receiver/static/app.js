...
(function () {
  ...
  function statusBadge(status) {
    var known = { open: "badge-open", waiting: "badge-waiting", failed: "badge-failed", blocked: "badge-blocked" };
    var s = String(status || "open").toLowerCase();
    return el("span", "badge " + (known[s] || ""), s);
  }
  ...
  function viewToday(anchor) {
    ...
    Promise.all([
      ...
      .then(function (results) {
        ...
        folio.appendChild(el("span", null, "Sotto — daily memo"));
        ...
        // Today's docket — the day's meetings (calendar), then research cards
        frag.appendChild(buildDocket(cal));
        var prepared = buildResearch(research);
        if (prepared) frag.appendChild(prepared);

        // On your desk — open loops that deserve attention today
        var loopRows = loops || [];
        var docket = el("div");
        var capRight;
        ...
        docket.appendChild(ledgerCap("On your desk", capRight));
        ...
        setView(errorView("Today's memo didn't load.", function () { viewToday(); }));
      });
  }
  ...
  function buildThesis(data, loops, cal) {
    ...
    if (data.bridge_connected === true) {
      ...
      parts.push(clause("your Mac hasn't checked in" + (ago ? " since " + ago.replace(" ago", "") + " ago" : " yet"), "/setup", true));
    }
    ...
    // 3 · The docket — one clause, only when today actually has meetings
    var todaysMeetings = splitDocketEvents(cal).today;
  ...
  /* today/tomorrow split on the event's own wall-clock date (the server already
     filtered to that window). Non-date starts stay in today — tolerant. */
  function splitDocketEvents(cal) {
    var out = { today: [], tomorrow: [] };
    ...
    for (var i = 0; i < events.length; i++) {
      ...
      (isDateOnly(prefix) && prefix > todayKey ? out.tomorrow : out.today).push(ev);
    }
    return out;
  }
  ...
  function attendeeNames(ev, selfEmail) {
  ...
  function meetingRow(ev, selfEmail) {
    var row = el("div", "ledger-row");
    var mainCol = el("div", "row-main");
    mainCol.appendChild(el("div", "row-title", str(ev.summary) || "Untitled meeting"));
    var names = attendeeNames(ev, selfEmail);
    if (names.length) mainCol.appendChild(el("div", "attendee-line", names.join(" · ")));
    row.appendChild(mainCol);

    var folio = el("div", "row-folio");
    folio.appendChild(el("span", null, fmtRange(ev.start, ev.end)));
    var link = str(ev.meeting_link);
    var loc = str(ev.location);
    var joinHref = isUrl(link) ? link : (isUrl(loc) ? loc : null);
    if (joinHref) {
      var a = el("a", "go", "join →");
      a.href = joinHref;
      a.target = "_blank";
      ...
      folio.appendChild(el("span", null, loc.length > 26 ? loc.slice(0, 25) + "…" : loc));
    }
  ...
  function buildDocket(cal) {
    ...
    if (cal && !cal.unavailable) {
      capRight = capCount(todays.length === 0 ? "clear"
        : todays.length === 1 ? "1 meeting" : todays.length + " meetings");
    }
    section.appendChild(ledgerCap("Today's docket", capRight));
    ...
    if (!cal) {
      ...
      var selfEmail = inferSelfEmail(todays.concat(tomorrows));
      var i;
      for (i = 0; i < todays.length; i++) {
        ledger.appendChild(meetingRow(todays[i], selfEmail));
      }
      if (tomorrows.length) {
        ...
        for (i = 0; i < tomorrows.length; i++) {
          var row = meetingRow(tomorrows[i], selfEmail);
  ...
  /* ---------------- Research cards (GET /api/research) ----------------
     A bonus surface: no research on file → render NOTHING (absence isn't a gap). */

  function buildResearch(research) {
    var entries = [];
    ...
    var section = el("div");
    var right = research.stale === true
      ? el("span", "cap-count", "from yesterday's research")
      : capCount(String(entries.length));
    section.appendChild(ledgerCap("Prepared for you", right));
    var ledger = el("div", "ledger");
    for (var r = 0; r < entries.length; r++) {
      ledger.appendChild(researchRow(entries[r]));
    }
    section.appendChild(enterOnce("today", ledger));
    return section;
  }
  ...
  function researchRow(entry) {
    var row = el("div", "ledger-row");
    var mainCol = el("div", "row-main");
    var email = str(entry.email).toLowerCase();
    var name = str(entry.name) || (email ? nameFromEmail(email) : "Unnamed");
    mainCol.appendChild(el("div", "row-title", name));

    // At most one Recently: line — the first recent_activity item
    ...
    // At most one Hook: line — the first conversation hook, italic serif
    ...
    var folio = el("div", "row-folio");
    var bits = [];
    if (str(entry.title)) bits.push(str(entry.title));
    if (str(entry.company)) bits.push(str(entry.company));
    if (bits.length) folio.appendChild(el("span", "folio-wrap", bits.join(" @ ")));
    row.appendChild(folio);
    return row;
  }
  ...
  function viewLoops() {
    ...
    api("/api/loops").then(function (data) {
      ...
      var frag = document.createDocumentFragment();
      frag.appendChild(el("p", "eyebrow", "Continuity"));
      ...
      frag.appendChild(ledgerCap("The ledger", counter.cap));
      ...
      if (seq !== state.renderSeq || err.handled) return;
      setView(errorView("The loop ledger didn't load.", viewLoops));
    });
  }
  ...
  function loopRow(loop, counter) {
    ...
    var folio = el("div", "row-folio");
    // The dot already says "open"; a badge earns its place only off the happy path.
    if (String(loop.status || "open").toLowerCase() !== "open") {
      folio.appendChild(statusBadge(loop.status));
    }
    var bits = [];
    if (loop.action_type) bits.push(String(loop.action_type).replace(/[_-]+/g, " "));
    if (loop.channel) bits.push(String(loop.channel));
    if (bits.length) folio.appendChild(el("span", null, bits.join(" · ")));
    var opened = timeAgo(loop.created_at);
    var surfaced = typeof loop.times_surfaced === "number" && loop.times_surfaced > 0
      ? "surfaced " + loop.times_surfaced + "x" : "";
    if (opened || surfaced) {
      folio.appendChild(el("span", null, [opened, surfaced].filter(Boolean).join(" · ")));
    }
    row.appendChild(folio);
  ...
  function viewBriefs(date, kind) {
    ...
    api("/api/briefs").then(function (data) {
      ...
      var frag = document.createDocumentFragment();
      frag.appendChild(el("p", "eyebrow", "Archive"));
      frag.appendChild(el("h1", "view-title", "Briefs"));
      frag.appendChild(el("p", "view-sub", "Every memo Sotto has delivered, kept on file."));
      ...
      if (seq !== state.renderSeq || err.handled) return;
      setView(errorView("The archive didn't load.", function () { viewBriefs(); }));
    });
  }
  ...
  function viewPeople(slug) {
    ...
    var frag = document.createDocumentFragment();
    frag.appendChild(el("p", "eyebrow", "Memory"));
    frag.appendChild(el("h1", "view-title", "People"));
    frag.appendChild(el("p", "view-sub",
      "One memory graph — the people you talk to and the companies around them. Open anyone to correct the record."));
  ...
  function personRow(p) {
    ...
    var folio = el("div", "row-folio");
    if (typeof p.fact_count === "number") {
      folio.appendChild(el("span", null, p.fact_count + (p.fact_count === 1 ? " fact" : " facts")));
    }
    var upd = timeAgo(p.updated_at);
    if (upd) folio.appendChild(el("span", null, upd));
  ...
  function renderPerson(seq, slug, data, flashText) {
    ...
    frag.appendChild(ledgerCap("On the record",
      capCount(active.length === 1 ? "1 fact" : active.length + " facts")));
    var list = el("div", "ledger");
    if (!active.length && !archived.length) {
      list.appendChild(emptyState(null, isCompany
        ? "Nothing recorded yet — research fills this page as briefs mention this company."
    ...
    if (isCompany) {
      frag.appendChild(el("p", "note",
        "Company pages are built by research. To correct one, tell Sotto in chat — it re-checks on the next brief."));
    }
    if (archived.length) {
      var details = el("details", "archived-facts");
      details.appendChild(el("summary", null, archived.length + " archived — superseded, kept for the record"));
  ...
  function renderFactDisplay(node, fact, ctx) {
    ...
    var conf = typeof fact.conf === "number" ? Math.max(0, Math.min(1, fact.conf)) : null;
    if (conf !== null) {
      var track = el("div", "conf-track");
      var fill = el("div", "conf-fill");
      fill.style.width = (conf * 100).toFixed(0) + "%"; // CSSOM, not a style attribute
      var sr = el("span", "visually-hidden", "confidence " + Math.round(conf * 100) + "%");
      track.appendChild(fill);
      mainCol.appendChild(track);
      mainCol.appendChild(sr);
    }
  ...
  function renderFactArchive(node, fact, ctx) {
    ...
    block.appendChild(el("p", "confirm-text", "Archive this fact? It stays on the record, out of the way."));
    var errEl = el("p", "inline-error");
    errEl.hidden = true;
    var row = el("div", "edit-row");
    var yesBtn, noBtn;
  ...
  function viewLearned() {
    ...
    api("/api/learned").then(function (data) {
      ...
      data = data || {};
      var frag = document.createDocumentFragment();
      // The nav is static markup (app.html), so The Record's entry point lives
      // here: a quiet folio link beside the eyebrow. #record also routes directly.
      var headRow = el("div", "head-row");
      headRow.appendChild(el("p", "eyebrow", "Calibration"));
      var recordLink = el("a", "folio-link", "the record →");
      recordLink.href = "#record";
      headRow.appendChild(recordLink);
      frag.appendChild(headRow);
      frag.appendChild(el("h1", "view-title", "Learned"));
      ...
      // The server lists style.json's top-level keys as buckets; bookkeeping
      // keys aren't registers, so they don't count and don't chip.
      var NOT_BUCKETS = { updated_at: 1, version: 1, schema_version: 1 };
      var rawBuckets = Array.isArray(style.buckets) ? style.buckets : [];
      var buckets = [];
      for (var bI = 0; bI < rawBuckets.length; bI++) {
        if (!NOT_BUCKETS[String(rawBuckets[bI])]) buckets.push(rawBuckets[bI]);
      }
      ...
      if (seq !== state.renderSeq || err.handled) return;
      setView(errorView("The calibration page didn't load.", viewLearned));
    });
  }
  ...
  function prefRow(spec, deleteRule) {
    ...
    var folio = el("div", "row-folio");
    var tagBits = [];
    if (spec.tag) tagBits.push(spec.tag);
    if (spec.extra) tagBits.push("→ " + spec.extra);
  ...
  /* ---------------- View: The Record (GET /api/ledger) ----------------
     The action timeline: outcomes.jsonl + dashboard audit rows, merged newest
     first by the server, grouped here by local day. Every row becomes a serif
     sentence; unknown shapes render whatever fields exist — never crash. */
  ...
  function viewRecord(daysPart) {
    ...
    api("/api/ledger?days=" + days).then(function (data) {
      ...
      var frag = document.createDocumentFragment();
      frag.appendChild(el("p", "eyebrow", "The record"));
      frag.appendChild(el("h1", "view-title", "What Sotto has done"));
      frag.appendChild(el("p", "view-sub",
        "Every action taken and every correction made, kept on file."));
      ...
      var list = el("div", "ledger");
      if (!entries.length) {
        list.appendChild(emptyState("Nothing on the record yet",
  ...
  function recordRow(entry) {
    ...
    folio.appendChild(el("span", null, d ? fmtClock(d) : "—"));
    if (entry.source === "outcome") {
      var bits = [];
      if (str(entry.channel)) bits.push(humanChannel(entry.channel));
      if (str(entry.tier)) bits.push(str(entry.tier).replace(/[_-]+/g, " "));
      if (bits.length) folio.appendChild(el("span", null, bits.join(" · ")));
    }
    row.appendChild(folio);
    return row;
  ...
  function dashboardSentence(e) {
    ...
    if (ev === "write") {
      ...
      if (m) {
        if (op === "correct") return "You corrected a fact on " + m[1];
        if (op === "archive") return "You archived a fact on " + m[1];
        if (op === "add") return "You added a fact on " + m[1];
        return "You edited the record on " + m[1];
      }
      if (ep === "/api/loops") {
