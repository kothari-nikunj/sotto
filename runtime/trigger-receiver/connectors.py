...
def _write_json(path: str, obj: dict, mode: int = 0o600) -> None:
    """Atomic (tmp + replace) 0600 write — a crash mid-write can't corrupt a token file, and the
    volume never holds a world-readable secret."""
...
def discover(service: str, force: bool = False) -> dict:
    ...
    try:
        _write_json(_discovery_path(service), disc)
    except OSError:
...
def ensure_client(service: str, redirect_uri: str, disc: dict | None = None) -> str:
    ...
    _write_json(_client_path(service),
...
def create_pending(service: str) -> tuple[str, str]:
    ...
    _write_json(os.path.join(_pending_dir(), f"{state}.json"),
...
def finish_auth(state: str, code: str, redirect_uri: str) -> dict:
    ...
    _write_json(token_path(service), record, 0o600)
    return record
