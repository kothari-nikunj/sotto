...
thread normally does the fetching and a dashboard hit within 10 minutes of a tick rides it free.
  ...
  3. **The post-meeting tap** (Editor Step 2 item 3 — the one ADDITIVE feature of the Editor week):
     ...
     ENDED since the last tick (≥ SOTTO_TAP_GRACE_MIN ago, so the user is actually out of the room)
...
wins when the environment names it), rooms (`resource.calendar.google`) are not people, and what's
...
Env: SOTTO_CALENDAR_REFRESH_SECS (default 900; 0 disables the daemon thread and the file entirely),
     SOTTO_MEETING_TAP (0 disables the post-meeting tap), SOTTO_TAP_GRACE_MIN (default 5),
     SOTTO_TAP_MAX_PER_DAY (default 3), SOTTO_TAP_LOOKBACK_MIN (default 2× the refresh cadence,
     floor 30 — how far back a tick will look for an end it hasn't handled yet),
     SOTTO_TAP_SKIP_INTERNAL (default 1 — skip internal-only standups/syncs).
"""
...
def _self_email(events) -> str:
    """The user's own address: SOTTO_USER_EMAIL when the environment names it (the SAME fallback
    compose_brief.py already uses for `userEmail`), else the docket's inference. The env matters
    for the post-meeting tap on a light day — inference needs at least two peopled events, and
    without an answer the user would be listed as an attendee of their own meeting."""
    return _s(os.environ.get("SOTTO_USER_EMAIL")).lower() or _infer_self_email(events)
...
def tap_grace_min() -> int:
    ...
    grace. Negative values are meaningless (a tap before the meeting ends); clamp at 0."""
    return max(0, _int_env("SOTTO_TAP_GRACE_MIN", TAP_GRACE_MIN_DEFAULT))
...
def tap_lookback_min() -> int:
    """How far back a tick looks for an unhandled end. Defaults to TWO refresh intervals (floor 30
    min) so a single skipped/slow tick still delivers the tap, while a receiver that boots at 4pm
    doesn't replay the whole day's meetings as a burst of nudges. Always at least a minute wider
    than the grace, so a window narrower than the grace can't silently disable the feature."""
    default = max(TAP_LOOKBACK_FLOOR_MIN, (refresh_secs() * 2) // 60)
    return max(tap_grace_min() + 1, _int_env("SOTTO_TAP_LOOKBACK_MIN", default))
...
def ended_meetings(events, now_utc: datetime, today: str = "") -> list:
    ...
    standups when SOTTO_TAP_SKIP_INTERNAL is on."""
    today = today or HOOKS["local_today"]()
    self_email = _self_email(events)
    grace, lookback = tap_grace_min() * 60, tap_lookback_min() * 60
    skip_internal = ((os.environ.get("SOTTO_TAP_SKIP_INTERNAL") or "").strip() or "1") != "0"
    out = []
    for ev in events:
...
def _save_tap_state(today: str, fired: list) -> None:
    """Atomic (tmp + replace) — written after EVERY dispatch, not once per tick, so a crash between
    two taps can never replay the first one."""
    path = tap_state_path()
    try:
        os.makedirs(os.path.dirname(path), exist_ok=True)
        tmp = path + ".tmp"
        with open(tmp, "w", encoding="utf-8") as f:
            json.dump({"date": today, "fired": fired}, f)
        os.replace(tmp, path)
    except OSError:
        pass
