...
CALENDAR_TTL_SECS = 600                # /api/calendar gather cache (the plan's 10-minute number)
...
def _login_page(msg: str = "") -> str:
    ...
    return (
        ...
        "h1 span{color:#1d6b50;font-style:normal}"
        ...
        "input:focus{outline:none;border-color:#1d6b50;box-shadow:0 0 0 3px rgba(29,107,80,.12)}"
        "button{margin-top:12px;width:100%;padding:11px;border-radius:8px;border:0;"
        "background:#1d6b50;color:#f4efe4;font-family:'Martian Mono',ui-monospace,monospace;"
        "font-size:12px;text-transform:uppercase;letter-spacing:.04em;font-weight:600;"
        "cursor:pointer}button:hover{background:#27775a}button:active{transform:scale(.98)}"
        ...
        "h1 span{color:#52b087}"
        "input{background:#191510;border-color:#362e20;color:#eae3d3}"
        "input:focus{border-color:#3f9d75;box-shadow:0 0 0 3px rgba(82,176,135,.15)}"
        "button{background:#3f9d75;color:#12100b}button:hover{background:#52b087}}"
...
def _local_today() -> str:
    """The user's local date: SOTTO_TIMEZONE / settings.json timezone via zoneinfo, else server
    local — mirrors the tz-resolution order compose_brief/brief_marker use for the brief filenames."""
    tz = _s(os.environ.get("SOTTO_TIMEZONE") or "")
    if not tz:
...
def api_learned() -> dict:
    """Style is summarized, never dumped: the fingerprint's canonical/recent buckets carry verbatim
    message samples, so only bucket NAMES and the per_person count leave this endpoint."""
    style = _read_json_file("style.json", default=None)
    summary: dict = {}
    if isinstance(style, dict):
        per = style.get("per_person")
        summary = {
            "buckets": sorted(k for k in style if k != "per_person"),
...
# One (ts, value) pair, module-level: a dashboard refresh must NOT fork a gather every time (the
# plan's 10-minute cache). The receiver reloads this module per process, so it starts empty.
...
def _run_calendar_gather():
    ...
    degrades to [] the same way — the Today view never 500s over Google."""
    try:
    ...
    if not script:
        return None
    raw = []
    with tempfile.TemporaryDirectory(prefix="sotto-dash-cal-") as td:
...
def api_calendar() -> dict:
    """GET /api/calendar → {events, generated_at, cached} (10-min cache), or
    ...
    can't make every dashboard refresh fork a 60s subprocess."""
    with _CAL_LOCK:
...
def api_ledger(days_param="") -> dict:
    ...
    event, …}), each row passed through verbatim plus source: "outcome"|"dashboard". Newest first,
    LEDGER_MAX_ROWS cap. Malformed lines and rows without a parseable ts are skipped."""
    try:
    ...
    for fname, source in (("outcomes.jsonl", "outcome"), ("dashboard_audit.jsonl", "dashboard")):
        ...
        for line in lines:
            ...
            if ts is None or ts < cutoff:
                continue
            entries.append({**rec, "source": source})
