...
# ── the key-based providers, in the SAME tile (one place for all connectors) ─────────────────────

def _setup_tail(tmp_path, monkeypatch, **env):
    monkeypatch.setattr(rec, "RAILWAY_DOMAIN", "myapp.up.railway.app")
    monkeypatch.setattr(rec, "MCP_TOKEN", "tok123")
    monkeypatch.setattr(rec.CONNECTORS, "DATA", str(tmp_path))
    monkeypatch.setattr(rec, "google_connected", lambda: (False, "nope"))
    for k in ("EXA_API_KEY", "PARALLEL_API_KEY", "GOOGLE_AI_API_KEY"):
        monkeypatch.delenv(k, raising=False)
    for k, v in env.items():
        monkeypatch.setenv(k, v)
    return rec._setup_page("abc").split("Connected services", 1)[1]


def test_key_providers_render_beside_the_oauth_ones(tmp_path, monkeypatch):
    """The whole point: Exa and Parallel are connectors too, and 'where do I see Exa?' should be
    answered in the place you'd look — the same tile as Granola, not a table in RAILWAY.md."""
    tail = _setup_tail(tmp_path, monkeypatch, EXA_API_KEY="k", GOOGLE_AI_API_KEY="g")
    assert "Granola (meeting notes)" in tail          # the OAuth kind, unchanged
    assert "Exa — connected" in tail
    assert "Parallel — not connected" in tail
    assert "PARALLEL_API_KEY" in tail                 # names the variable that would turn it on


def test_the_page_never_renders_a_key(tmp_path, monkeypatch):
    """Read-only means read-only: the tile says WHETHER a key is set, never what it is. Anyone who
    can see the dashboard can see this page."""
    # Short, obviously-fake values on purpose: the publish SECRETS guard flags any `…KEY=` followed
    # by 12+ secret-alphabet characters, and it is right to — a test fixture that LOOKS like a real
    # key is how one eventually is one.
    tail = _setup_tail(tmp_path, monkeypatch, EXA_API_KEY="e-fake", PARALLEL_API_KEY="p-fake")
    assert "e-fake" not in tail and "p-fake" not in tail


def test_the_rendered_ladder_matches_which_provider_would_actually_answer(tmp_path, monkeypatch):
    """The page's loudest claim is precedence. With Parallel unset, deep research falls to Exa —
    and the page must say Exa, because a page that names a provider the ladder wouldn't use is a
    confident wrong answer about what Sotto can do."""
    tail = _setup_tail(tmp_path, monkeypatch, EXA_API_KEY="k", GOOGLE_AI_API_KEY="g")
    assert "deep research: Exa → Gemini grounding" in tail
    assert "web search: Exa → Gemini grounding" in tail
    # …and with Parallel set it leads deep research, without touching web search.
    tail = _setup_tail(tmp_path, monkeypatch, EXA_API_KEY="k", PARALLEL_API_KEY="p",
                       GOOGLE_AI_API_KEY="g")
    assert "deep research: Parallel → Exa → Gemini grounding" in tail
    assert "web search: Exa → Gemini grounding" in tail


def test_no_provider_connected_says_so_rather_than_showing_an_empty_ladder(tmp_path, monkeypatch):
    """Fail toward silence in pipelines, but a source that can't work must SAY so (CLAUDE.md)."""
    tail = _setup_tail(tmp_path, monkeypatch)
    assert "web search: <b>nothing connected</b>" in tail
    assert "deep research: <b>nothing connected</b>" in tail
