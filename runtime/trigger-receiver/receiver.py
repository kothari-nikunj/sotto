...
def _delivery_channel_ready(label: str) -> bool:
    """True when a nudge dispatched now could actually be delivered. Logs one line per state change
    (never per tick) so 'why did my nudges stop?' is answerable from the deploy log."""
    if (os.environ.get("SOTTO_CRON_DELIVER") or "whatsapp").strip() != "whatsapp":
        return True
    ok = _whatsapp_status() == "linked"
...
# from, and when they differ the Integrations page says so — the update itself is Railway's redeploy
...
PUBLIC_REPO = "kothari-nikunj/sotto"        # the canonical distribution repo (tools/sync-public-repo.sh)
...
def check_for_update() -> dict:
    ...
    cache ({latest, fetched_at}). A failed fetch keeps the previous answer rather than erasing it —
    the flag must never flicker off because GitHub was briefly unreachable."""
    ...
    if not latest:
        return cached
    fresh = {"latest": latest, "fetched_at": time.time()}
    path = _cache_file(UPDATE_CACHE_FILENAME)
    try:
...
def _setup_page(code: str = "") -> str:
    ...
    # Version facts, in the same quiet mono zone as the Host line at the foot of the page — this is
    # the ONE place Sotto ever mentions being out of date (no nudge, no brief line, no banner). The
    # update line renders only when a newer stamp is published; both stay absent on a dev build.
