    ...
    if verdict.get("verdict") == "agent":
        try:
            ...
            with open(bundle_path, "w", encoding="utf-8") as f:
                json.dump(verdict.get("bundle") or {}, f)
        except OSError as e:
...
def _poll_gmail_once() -> list:
    """One poll_gmail.py run → email events ([] on any failure; the script is itself fail-silent,
    this guard covers exec/parse)."""
...
def _gmail_poll_loop(secs: int) -> None:
    ...
    while True:
        try:
            ...
            print(f"[sotto] gmail poll error: {e}", flush=True)
...
def _connect_page(heading: str, body_html: str) -> str:
    return (
        ...
        "<title>Sotto — connect a service</title>"
        "<style>body{font-family:system-ui;max-width:640px;margin:40px auto;padding:0 16px;"
        "line-height:1.5;color:#111}code{background:#f3f3f3;padding:1px 4px;border-radius:3px;"
        "word-break:break-all}</style></head><body>"
        f"<h2>{heading}</h2>{body_html}"
        "<p><a href='/setup'>← back to setup</a></p></body></html>"
    )
...
def _connect_error_page(step: str, detail: str) -> str:
    ...
    return _connect_page(
        ...
        f"<p style='color:#666'>{_html.escape(detail)}</p>")
...
def _setup_page(code: str = "") -> str:
    ...
    for s in CONNECTORS.service_status():
        ...
        if s["connected"]:
            ...
            svc_rows.append(f"<p>{lbl} — <span style='color:#1a7f37;font-weight:700'>"
                            f"✓ Connected{_html.escape(when)}</span></p>")
        else:
            svc_rows.append(
                f"<p>{lbl} — <a class='btn' href='/connect/{s['service']}/start{qs}'>Connect →</a> "
                "<span style='color:#666;font-size:13px'>(one click — approve in your browser; "
                "tokens stay on your volume)</span></p>")
    svc_connected = any(s["connected"] for s in CONNECTORS.service_status())
    svc_badge = badge(True) if svc_connected else "<span style='color:#999;font-weight:700'>○ optional</span>"
    services = ("<p style='color:#666;font-size:13px;margin:2px 0 6px'>Optional extras — e.g. Granola "
    ...
    return (
        ...
        "<h2>Set up Sotto</h2><p style='color:#666;margin-top:0'>Your agent is live. Five tiles, all on this page (the fifth is optional).</p>"
        ...
        f"<div class='row'><h3>5 · Connected services</h3>{svc_badge}</div>{services}"
        f"{footer}"
