...
def _setup_page(code: str = "") -> str:
    ...
    mac = (f"<p><a class='btn-primary' href='{el}'>Open in Sotto Bridge →</a></p>"
           ...
           "<p class='tile-status'>Your Mac is linked and reachable. (Grant Full Disk Access in the app if you haven't.)</p>"
           + ev_line)
    ...
    wa = ("<p class='tile-status'>Pairing in progress — "
          ...
          else f"<p><a class='btn-primary' href='/whatsapp/qr{qs}'>Show WhatsApp QR →</a> <span class='tile-hint'>"
    ...
    # page's FIRST affordance becomes the handoff to the dashboard. WhatsApp has no positive linked
    # probe (see _whatsapp_status), so "done" there means "not mid-pairing".
    done = (st["bridge_connected"] and st["google_connected"] and bool(st["timezone"])
            and st["whatsapp"] != "pairing")
