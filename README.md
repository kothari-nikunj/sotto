...
**Real-time events (Phase 2).** Beyond schedules, the Bridge also *pushes* events as they happen: a watcher ticks the local stores every few seconds and POSTs new iMessages/WhatsApp messages/calls to the receiver's `/bridge/events`, where a three-tier triage funnel (deterministic drop/queue → a cheap Flash-Lite classifier → the `sotto-event` skill) turns only the genuinely interruption-worthy ones into a single nudge **with a ready-to-send draft** — never auto-sent. Inbound email joins the same funnel via cloud-side Gmail polling (no Pub/Sub setup), so email events work even without a new Bridge; older Bridges simply never POST events (the endpoint is additive — zero breakage). Quieter survivors queue for an adaptive 12:30 catch-up digest that stays silent on light days. Knobs: `SOTTO_EVENTS`, `SOTTO_EMAIL_POLL_SECS`, `SOTTO_DIGEST`, … (see [RAILWAY.md](RAILWAY.md)).

## The model: a 1M-context Gemini model

The brief's big multi-source extraction needs a **1M-context model**, and `compose_brief.py` calls the **Gemini API directly** (via `GOOGLE_AI_API_KEY` — the same key the host manages; no second key store). Get a **Gemini API key** ([aistudio.google.com/apikey](https://aistudio.google.com/apikey)). The default is **Gemini 3.6 Flash** (`gemini-3.6-flash` — 1M, stronger tool use, a great general driver too); on a quota/5xx error the brief **automatically falls back** to `gemini-3-flash-preview` (cheaper, and a separate rate-limit bucket — same key). Override the model with `SOTTO_GEMINI_MODEL`, the backup with `SOTTO_FALLBACK_MODEL` / `SOTTO_FALLBACK_API_KEY` (a second key from another project dodges per-project quota; keep any override 1M-context). (Non-Gemini providers aren't wired into the brief's direct call today — the fallback is Gemini-API-shaped.)
...
1. **Deploy** the host on Railway — **one click** via the Deploy button in [ONBOARDING.md](ONBOARDING.md) / [RAILWAY.md](RAILWAY.md) (it sets up the build, the `/data` volume, `BRIDGE_TOKEN`, and the domain; you add only your Gemini key + WhatsApp number — the number is entered twice, once as the allowlist and once as the delivery channel), or the four manual settings if you'd rather wire it by hand. The container installs Hermes, the skills, the persona, and the morning/evening crons for you.
...
That's it — everything after is conversational ("who am I meeting?", "draft Dhruv", "find 30 min with Alex next week", "accept my 3pm", "who am I losing touch with?").
