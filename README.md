...
2. **Link your Mac** — [download Sotto Bridge from Releases](https://github.com/kothari-nikunj/sotto/releases/latest),
   ...
   click from the `/setup` page it opens. A 3-step wizard in the app covers disk access and privacy
   toggles. Connect Google + scan the WhatsApp QR on the same setup page.
...
**Staying updated:** your server checks once a day and flags a newer Sotto on its `/setup` page —
merge Railway's update PR (or hit **Sync fork** on GitHub) and the redeploy is the update, Hermes
included. Details: **[RAILWAY.md § Staying updated](RAILWAY.md#staying-updated)**.
...
like texting it), and the voice + preference rules it has learned. Text stays the primary
interface; the dashboard is the window.
