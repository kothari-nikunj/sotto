...
- **Real-time nudges with drafts** — the Bridge streams new messages/calls within seconds; a triage
  funnel nudges only for the genuinely urgent (reply pre-drafted, one tap), everything else lands in
  an adaptive midday digest or the next brief. Inbound email joins via cloud-side polling.
