...
app isn't on this Mac — and the command exits 0 only when every enabled source is ok.
...
creates the brief crons.

> **This repo ships no Bridge source**, so `install.sh` can't auto-detect a built binary for local
> mode — register the downloaded app's bundled engine yourself. Use `configure_mcp.py`'s `--command`
> flag (the same call `install.sh` makes internally) pointed at the app's bundled `sotto-bridged`:
>
> ```bash
> python3 adapters/hermes/configure_mcp.py --name sotto-local \
>   --command "/Applications/Sotto Bridge.app/Contents/Resources/sotto-bridged"
> ```
