...
- A **1M-context model key** — a Google **Gemini API key** ([aistudio.google.com/apikey](https://aistudio.google.com/apikey);
  Gemini 3.7 Flash is cheap, 1M, and a fine driver). Sotto stores no keys — this goes in Hermes' native
...
```
`--dedicated` sets the chat model to the native `gemini-3.7-flash` — the right default for a
