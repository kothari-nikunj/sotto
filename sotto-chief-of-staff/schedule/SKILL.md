...
2. **Propose times** — reason over the busy blocks in `/tmp/sotto_cal.json` to find **free slots** that fit the ask (duration, the user's preferred window, working hours, timezone). There is no free/busy API — you compute availability from the events. Offer **2–3 concrete options** with explicit dates + times + timezone. Never propose a slot that overlaps an existing event.
   - **Resolve the place in the same proposal.** A bare venue name ("Blue Bottle", "Equinox") books a
     string Google Maps guesses at per-attendee — each invitee can be pointed at a DIFFERENT branch.
     Before proposing: (a) check the gathered calendar for past events at that venue and reuse their
     full location string; (b) else resolve to one full address with a single grounded lookup —
     `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/web_research.py" "Blue Bottle Coffee near
     Ferry Building San Francisco address"` (bias the query with whatever geography you know: the
     user's city/timezone, the other attendee's company location, surrounding events that day);
     (c) if it's genuinely ambiguous ("Blue Bottle" in a city with ten), ask WHICH one as part of the
     time proposal — one question, not two rounds. The proposal the user approves must show the full
     address, so approval covers the time AND the exact place. A unique venue or full address the
     user typed passes through verbatim — don't "improve" it.
3. **On approval, book it** — `execute_code` → `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/google_action.py" calendar-create --summary "<title>" --start <ISO-with-offset> --end <ISO-with-offset> --attendees "a@x.com,b@y.com" --location "<place, if the user gave one>" --description "<agenda note, optional>"`. It returns `{status:"created", id, summary, htmlLink}`. Confirm with the `htmlLink`. If the result carries `location_attached: false`, the host's CLI couldn't attach the place — say so honestly and include the address in your confirmation message (never silently drop it, never stuff it into the title).
