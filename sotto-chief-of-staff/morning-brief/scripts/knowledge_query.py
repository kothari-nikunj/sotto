...
Usage:
    ...
    knowledge_query.py --calendar /tmp/sotto_cal.json   # + always pack today's attendees
...
def main():
    ...
    ap.add_argument("--calendar", help="gathered calendar JSON; its attendees always pack, "
                                       "regardless of the --relevant-days mtime window")
    args = ap.parse_args()
    now = datetime.now()
    ...
    for path in glob.glob(os.path.join(kg.people_dir(), "*.md")):
        ...
        # mtime gate — but today's cast is EXEMPT: mtime only says when the file was last rewritten
        # (persist_prep.py documents why that's the wrong freshness signal), so someone you're meeting
        # after a three-week gap would pack zero context without the calendar exemption.
        if not any(i.strip().lower() in cal_emails for i in identifiers):
            try:
