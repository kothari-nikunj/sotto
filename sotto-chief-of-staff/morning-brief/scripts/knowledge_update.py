...
def apply(extracted: dict, now: datetime | None = None) -> dict:
    ...
    for upd in extracted.get("person_updates", []):
        ...
        if not cid:
            seed = f"kf:{name}|{ident}" if ident else f"kf:{name}"
            if not name and not ident:
                continue
            cid = kg.generate_canonical_id(seed)

        slug = _slug_for(name or cid)
        if not slug:
            continue  # no safe filename → skip rather than write outside the dir
