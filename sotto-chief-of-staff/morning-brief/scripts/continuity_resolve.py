...
from textutil import unwrap_tool_result  # noqa: E402  (_shared/lib is on sys.path via compose_brief)
...
def _normalize_action_type(t: str) -> str:
    ...
    the occasional hyphen/space spelling of follow_up / call_back. Both anchor-keying AND
    cross-channel resolution key off this, so a variant that isn't normalized never dedupes with
    its canonical twin and never self-resolves."""
    t = (t or "").lower().strip().replace("-", "_").replace(" ", "_")
    ...
    for raw in payload.get("new_actions", []):
        ...
        if ak in items:
            ...
            if it.get("status") in TERMINAL:
                ...
                it["created_at"] = a.get("created_at") or today   # fresh ask — restart the age clock
