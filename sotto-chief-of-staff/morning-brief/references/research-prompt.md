...
## The two passes (both batched, both Gemini Search Grounding, run concurrently)
...
The quality upgrade: a separate grounded call that hunts the last `SOTTO_RESEARCH_RECENCY_DAYS`
(default 90) days for what is genuinely NEW — the model is instructed to run MULTIPLE distinct
...
- **conversation_hooks** (≤2) — one-line openers grounded ONLY in the finds ("He published a piece
  on agent memory last week — strong hook given your roadmap").
...
Env knobs: `SOTTO_RESEARCH_DEEP=0` disables Pass B entirely; `SOTTO_RESEARCH_RECENCY_DAYS`
adjusts the window.
...
- **Deterministic post-filters** (in `research_attendees.py`, not just the prompt): a
  ...
  explicit public source URL in the string is dropped; a `conversation_hook` matching generic
  process filler ("ask for introductions", "understand the agenda", "explore potential overlaps",
  "build rapport") with no concrete anchor is dropped. "If nothing genuinely recent exists, return
  ...
  "attendees": [
    {
      ...
      "conversation_hooks": ["His agent-memory Substack piece last week overlaps your knowledge-graph roadmap — strong opener."]
    }
  ]
}
```
...
labeled `source: "web_research"` — never as authoritative identity fields. `conversation_hooks`
are ephemeral meeting-day material and are never persisted.
