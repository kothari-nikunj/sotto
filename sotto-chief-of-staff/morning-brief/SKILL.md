...
4. **Learn — ONE command runs all six memory writers (knowledge, continuity, draft outcomes, style, meeting attendance, contacts) and leaves a receipt.** Write the two payload files first, then run the step. A brief that delivers without this receipt is logged as a brief that did not learn — the receiver checks for it when the send is acknowledged.
   ...
   2. Write `{ "today": "<YYYY-MM-DD>", "signals": { "replied_thread_ids": [<gmail/thread ids you replied to today>] }, "new_actions": <the brief's `actions[]` verbatim>, "local": <the read_local JSON from /tmp/sotto_local.json>, "emails": <emails from /tmp/sotto_gmail.json>, "events": <calendar events from /tmp/sotto_cal.json> }` to `/tmp/sotto_cont.json` (pass the brief's `actions[]` **as-is** — the script reads the FLEX camelCase fields directly).
   3. `execute_code` →
      ```bash
