...
4. **Learn — run ALL SIX scripts below (knowledge, continuity, preferences, style, meeting attendance, contacts) so Sotto's memory actually accumulates; skipping any of them means the next brief starts that much colder.** Use absolute paths.
   ...
   3. **Preferences (behavioral learner)** — `execute_code` →
      ...
      (first matches yesterday's offered drafts — `events/drafts.jsonl`, written whenever a tap link is built — against what the user actually SENT (the queue's `is_from_me` signals): sent-verbatim / edited / dismissed land in `$SOTTO_DATA/outcomes.jsonl`, and a verbatim send confirms its style sample. Then tallies outcomes → `preferences.json`: deprioritization hints + the per-(contact, action_type) `approval_defaults` that `_shared/references/approval-tiers.md` governs. Fast and idempotent; a missing/empty outcomes log is a no-op, and the user's `explicit` preferences block is never touched.)
   ...
   5. **Meeting attendance (who you actually sat with)** — `execute_code` →
      `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/granola_graph.py" --granola /tmp/sotto_granola.json`
      (writes one dated, sourced `Met on <date>: <title>` fact onto each attendee of every meeting with 8 or fewer attendees — a bigger event is not a relationship — and fills a blank `company` from a corporate email domain. Re-running the same meeting bumps the fact instead of duplicating it; a missing file is a no-op.)
   6. **Contacts (identity, notes, birthdays)** — `execute_code` →
      `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/prewarm_graph.py" --sync-contacts`
      (re-syncs from the snapshot the brief already saved — no live Contacts read: every email AND phone on a card becomes an identifier on that person's file, and the card's `notes` and `birthday` become sourced facts. Only cards Sotto already knows or that carry a note/birthday sync; an address book is a directory, not a graph.)
