...
1. **Gather (deterministic, fast):**
   ...
   - Continuity open-loops — deterministic read (never hand-read `$SOTTO_DATA/knowledge/continuity/*.md`; `loops_query.py` is the only sanctioned read view): `execute_code` → `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/loops_query.py" > /tmp/sotto_loops.json`. It prints `{you_owe:[…], waiting_on_them:[…], counts}` where each item is `{name, what, channel, identifier, deadline, …}`. Then write `/tmp/sotto_cont.json` in the shape `proactive_scan.py` expects — a **bare JSON LIST** (not the loops_query dict!), one entry per item across `you_owe` + `waiting_on_them`, each mapped as `{"title": "<name> — <what>", "deadline": <deadline>, "channel": <channel>, "identifier": <identifier>}`. Only items with a `deadline` can produce a nudge. Skip this input entirely if both lists are empty.
