...
3. **For each nudge, follow `_shared/references/approval-tiers.md` and deliver ONE concise message.**
   ...
   - `meeting_prep` → **give useful context before offering the full prep.** Name the person, their known role and the time; never invent a title or introduction. Add one or two short sentences about the introduction chain, the reason for this meeting or a relevant last exchange, only when supported by memory, the invitation or dated correspondence. An organizer is not necessarily an introducer, and a past discussion is not automatically today's agenda. The direct composer uses the existing bounded notification writer and the focused prep's Gmail lookup for the selected attendee only; no web research runs for the reminder. Unknown context, unavailable sources or a held/failed writer fall back to the basic reminder. Include the existing `open_loop` if present, then one named question: "Want the full prep on Spencer Kim?" Naming the person makes a bare "yes" answerable: a yes runs `sotto-meeting-prep` focused on that person, with no list of the user's other meetings. The full prep uses the four-photo iMessage gallery when eligible; the reminder stays short text.
...
- **One rulebook, not two — structurally.** `proactive_scan.py` decides only WHAT is due and then
  ...
  held, writes the SAME `surfaced.jsonl` row, including the ones the clock suppressed. A `meeting_prep` nudge is skipped only when a provider-accepted prep offer or full prep records the same calendar event ID and start; research alone never suppresses it, and a reschedule is a new occurrence. The **lead**
