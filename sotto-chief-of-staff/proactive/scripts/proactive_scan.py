...
Four nudge kinds:
  ...
  - retune_offer  — the stale-loop pile is getting heavy (you keep seeing items you don't act on);
                    offer a quick cleanup (sotto-retune). Throttled to once per cooldown window, NOT
                    daily — and suppressed within 2h of a delivered brief (Sprint 0 §6): the brief
                    just listed those very loops, so an immediate "your list is heavy" reads as a
                    duplicate, not help.
...
while it is in the future, exactly like quiet hours and exactly as the event funnel honors it.
...
def _snooze_until() -> str:
    """The user's explicit nudge snooze stamp (preferences.explicit.nudge_snooze_until), written by
    the sotto-feedback verbs. Best-effort — no preferences file means no snooze."""
    try:
        import preferences as _prefs  # noqa: PLC0415  (sibling in _shared/scripts, on sys.path)
        return str(_prefs.load_explicit().get("nudge_snooze_until") or "").strip()
    except Exception:
        return ""


def _snooze_active(until: str, now_local: datetime) -> bool:
    """ONE rule, shared with triage_event.py: preferences.snooze_active. Any failure → not snoozed
    (a broken stamp must never silence the watcher forever)."""
    try:
        import preferences as _prefs  # noqa: PLC0415
        return bool(_prefs.snooze_active(now_local=now_local,
                                         explicit={"nudge_snooze_until": until}))
    except Exception:
        return False
...
def scan(calendar, continuity, local, user_email, now_local,
         ...
         snooze_until: str = "") -> dict:
    """Pure decision (no I/O): given the inputs and the local 'now', return the nudges due now.
    Quiet hours — and the user's explicit nudge snooze — suppress everything. Dedup is applied by
    the caller (main). `stale_count` / `retune_offer_allowed` / `snooze_until` are computed by main
    (they need disk: the ledger, a cooldown marker, preferences.json)."""
    ...
    # The same cadence lever the event funnel honors ("quieter today" / "quiet until 3"): while the
    # snooze stamp is in the future, the watcher is silent. `quiet: true` keeps main from marking
    # anything as nudged, so a meeting-prep due during the snooze can still fire once it lifts (if
    # it is still in the lead window) rather than being burned by the dedup state.
    if snooze_until and _snooze_active(snooze_until, now_local):
        return {"nudges": [], "quiet": True, "reason": f"nudges snoozed until {snooze_until}"}
...
def main():
    ...
    now_local = cb._now_local(cb.configured_tz() or "+00:00")
