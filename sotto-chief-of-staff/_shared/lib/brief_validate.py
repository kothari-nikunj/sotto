...
Rules implemented (each returns a one-line violation string):
  ...
  d. Coming Up is capped at 5 content lines.
...
def _check_coming_up_length(markdown: str) -> list:
    count = 0
    for line, _section, in_coming_up in _split_lines_with_sections(markdown):
        ...
        count += 1
    if count > COMING_UP_MAX_LINES:
        return [f"coming-up-overflow: {count} content lines (max {COMING_UP_MAX_LINES})"]
    return []
...
def validate(brief_markdown: str, action_items: list, rendered_source_text: str) -> list:
    ...
    try:
        ...
        violations += _check_coming_up_length(brief_markdown)
