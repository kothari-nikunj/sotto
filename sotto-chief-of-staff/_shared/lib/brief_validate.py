...
Rules implemented (each returns a one-line violation string):
  ...
  g. every open/waiting action_ledger entry is NAMED somewhere in the brief — the open-items
     contract: an open loop appears as an ask or in Already Handled, never silently dropped.
...
def _is_name_shaped(name: str) -> bool:
    ...
    label the brief has no reason to print verbatim, so it can never be the evidence token."""
    words = name.split()
    return 1 <= len(words) <= 3 and all(_NAME_WORD_RE.match(w) for w in words)


def _ledger_token(entry: dict) -> str:
    """What must appear in the brief for this ledger entry to count as surfaced: the contact's name
    when it is name-shaped, else the longest 5+ character word of its summary (a rare word is the
    cheapest evidence that THIS loop, not some other, got mentioned). An entry with neither has no
    honest token, so it is never reported missing — a token the brief was never going to contain
    would print the same "## Still open" line under every brief, forever."""
    name = str(entry.get("contact_name") or "").strip()
    if name and _is_name_shaped(name):
        return name
...
def missing_open_loops(brief_markdown: str, action_ledger: list) -> list:
    """The open/waiting ledger entries the brief never names — the raw list behind rule (g), exposed
    so compose_brief's deterministic backstop can render exactly the misses this rule flags."""
    low = (brief_markdown or "").lower()
    out = []
    for e in action_ledger or []:
        if not isinstance(e, dict) or e.get("status") not in ("open", "waiting"):
            continue
        token = _ledger_token(e)
        if token and not _token_present(token, low):
            out.append(e)
    return out


def _check_open_ledger_coverage(markdown: str, action_ledger: list) -> list:
    return [f"dropped-open-loop: open ledger item '{_ledger_token(e)}' "
            f"({str(e.get('summary') or '')[:60]}) never appears in the brief — every open item is an "
            f"ask with its age, or Already Handled with the evidence"
            for e in missing_open_loops(markdown, action_ledger)]


def validate(brief_markdown: str, action_items: list, rendered_source_text: str,
             first_run: bool = False, action_ledger: list | None = None) -> list:
    ...
    brief was built from (rule g); omit it and that rule simply doesn't run."""
    violations = []
    try:
        ...
        violations += _check_open_ledger_coverage(brief_markdown, action_ledger)
