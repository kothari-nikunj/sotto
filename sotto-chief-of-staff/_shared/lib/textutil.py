...
def unwrap_tool_result(obj, data_keys=_LOCAL_DATA_KEYS):
    ...
    (default: the LocalData markers) is found. Shared by compose_brief, style_extract,
    continuity_resolve and prewarm_graph so every read_local consumer unwraps identically —
    a wrapper reaching only ONE of them silently zeroed the others' ingestion."""
    for _ in range(5):
