...
This module is the OWNER of these names: every consumer (compose_brief and its ~14 sibling
scripts) imports them from here, so there is one implementation and one import path.
"""
...
# The keys that mark a dict as ALREADY being the LocalData payload (not an MCP wrapper around it).
_LOCAL_DATA_KEYS = ("imessage", "source_status", "generated_at", "whatsapp", "contacts")


def unwrap_tool_result(obj, data_keys=_LOCAL_DATA_KEYS):
    """Tolerate a RAW MCP tool-result wrapper around a JSON payload (the shape the agent gets from
    `read_local` and dumps to disk AS-IS, per morning-brief/SKILL.md). Peels
    {result}/{structuredContent}/{content:[{text}]} layers until a dict carrying any of `data_keys`
    (default: the LocalData markers) is found. Shared by compose_brief, compose_meeting_prep,
    compose_followup, triage_queue, proactive_scan, style_extract, continuity_resolve and
    prewarm_graph so every read_local consumer unwraps identically — a wrapper reaching only ONE of
    them silently zeroed the others' ingestion. It also means no consumer needs a brittle
    `python3 -c` to unwrap (which trips the dangerous-command gate and kills headless briefs)."""
    for _ in range(5):
        if not isinstance(obj, dict):
            return {}
        if any(k in obj for k in data_keys):
            return obj  # already the payload
...
def normalize_attendee_research(v) -> list:
    """Accept BOTH attendee-research shapes: the documented bare list, and the {"attendees":[…]}
    envelope research_attendees.py --out actually writes (which every skill passes straight through
    as --attendee-research). Without this the dict shape read as [] in the renderer and the whole
    research block silently vanished — from the brief, and then, unfixed, from meeting prep.
    ONE implementation, here in the leaf, so a third consumer can't be wrong a third time.
