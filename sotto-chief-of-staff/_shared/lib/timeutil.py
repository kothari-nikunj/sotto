...
It also holds `configured_user_email()` — the "who am I" chain (SOTTO_USER_EMAIL →
settings.json:google_account_email) — for the same reason: it reads the same settings file.

This module is the OWNER of these names — compose_brief and every sibling that needs the user's
local day import them from here.
"""
...
def configured_user_email() -> str:
    """The user's own address: SOTTO_USER_EMAIL when the environment names it, else the
    `google_account_email` the Google connect derived onto the volume (the receiver writes it at
    OAuth-exchange success — the `From` of an `in:sent` message IS the authorized account).

    This is the CANONICAL chain and the ONE copy of it in the skills tree — compose_brief's
    self/user-email fallbacks and google_action's guest-list self-append both call it, and the
    receiver's calcache._self_email mirrors it (env → settings → its own inference). Sotto learns
    your address from the account you connected; the env var is an OVERRIDE, not a requirement.
    (Lives in timeutil because load_settings — the settings.json reader — already does.)"""
    return (_s(os.environ.get("SOTTO_USER_EMAIL"))
            or _s(load_settings().get("google_account_email"))).lower()
