...
def _format_action_ledger(local, now=None) -> str:
    ...
    # list was built), so placement and phrasing are the model's call — inclusion is not.
    return ("## Open Commitments (ACTION LEDGER from previous briefs)\n"
            ...
            "- EVERY entry below appears in the brief exactly once — as an ask with its age if the user\n"
            "  has not acted on it, or in Already Handled naming the evidence if today's data shows it\n"
            "  fulfilled. Dropping one silently is a defect, not an editorial choice.\n"
