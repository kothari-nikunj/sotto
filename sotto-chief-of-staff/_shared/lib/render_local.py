...
This module is the OWNER of these names — compose_brief, triage_queue, compose_meeting_prep and
select_attendees import them from here.
"""
...
def build_contact_lookup(contacts: list) -> dict:
    # NO 7-digit keys: two contacts in different area codes can share the last 7 digits, and a
    # confident wrong name is worse than a raw number (the misattribution bug). 10-digit suffix
    # (country-code tolerant) is the shortest match we trust.
    lookup: dict[str, str] = {}
    for c in contacts:
        name = _s(c.get("name"))
        if not name:
...
def resolve_whatsapp_name(jid: str, partner_name: str, lookup: dict) -> str:
    """Apple Contacts is AUTHORITATIVE: a Contacts match on the JID's phone wins over WhatsApp's
    partner_name (which for a 1:1 can be the peer's self-set push name — any string they chose).
    Only when Contacts has nothing do we trust partner_name, then fall back to the number.
    @lid JIDs are WhatsApp privacy ids, NOT phone numbers — their digits must never be looked up
    against Contacts (false last-10 matches) or rendered as a phone number (a fabricated number)."""
    jid = _s(jid)
    is_lid = jid.endswith("@lid")
    d = "" if (is_lid or not jid) else _digits(jid.split("@")[0])
    if jid:
        if lookup.get(jid):
            return lookup[jid]
        if not is_lid:
    ...
    # LOSSY last-10 fallback sits BELOW partner_name: it strips the country code, so a non-US
    # number can false-match a US contact on the same trailing 10 digits (+81 80-1234-5678 vs
    # +1 801-234-5678). Only exact-form Contacts matches may override the peer's push name.
    if len(d) >= 10 and lookup.get(d[-10:]):
        return lookup[d[-10:]]
    if not jid or is_lid:
        return "Unknown"
    return _format_phone_for_display(d)
...
def seed_contact_index_from_contacts(contacts: list) -> list:
    """Apple Contacts → canonical-index entries. THE phone↔email bridge for saved contacts: every
    identifier (all phones AND all emails) of one card maps to ONE canonical_id, so the same human
    texting from a phone and emailing from an address collapses into a single identity downstream.
    canonical_id uses the knowledge-graph scheme (sha256 of 'kf:{name}|{first_email}' / 'kf:{name}',
    same as knowledge_update/style_extract/prewarm_graph) so ids line up with people/*.md files."""
    import hashlib
    out = []
    for c in contacts or []:
        ...
        if not name:
            continue
        emails = [_s(e).strip().lower() for e in _arr(c, "emails") if _s(e).strip()]
        phones = [_s(p).strip() for p in _arr(c, "phones") if _s(p).strip()]
        if not emails and not phones:
            continue
        seed = f"kf:{name}|{emails[0]}" if emails else f"kf:{name}"
        cid = "c_" + hashlib.sha256(seed.encode()).hexdigest()[:12]
        out.append({"canonical_id": cid, "display_name": name,
                    "identifiers": emails + phones, "confidence": "high"})
    return out
...
def _process_wa_missed_calls(calls, lookup, connected):
    ...
    for call in calls:
        ...
        if not name or name == "Unknown" or name.startswith("(") or name.startswith("+"):
            continue
        t = _parse_ts(_s(call.get("timestamp")))
        if p in connected and t and connected[p] > t:
            continue
...
def build_identity_resolver(local: dict):
    """THE identity resolver: identifier → {name, canonical_id, confidence}, or None.

    Fed BOTH the knowledge-graph contact_index (medium confidence — LLM/graph-derived identifiers)
    AND an index seeded from Apple Contacts (high confidence, listed last so a Contacts identity
    wins any identifier collision).

    It exists as a named function because it has TWO consumers and they must answer identically:
    `resolve_contact_names` names 1:1 threads with it, and `_group_messages_into_threads` names a
    GROUP's participants with it. When only the first had it, a group of people Sotto knew from
...
def _pick_name(identity, fallback):
    """Graph identity vs. the Contacts/WhatsApp fallback, one precedence for every caller.

    A high-confidence identity (Apple Contacts) always names the thread. A medium-confidence
    identity (knowledge graph) names it ONLY when the fallback isn't a real name — a graph match on
    an exact identifier beats showing the user a raw phone number, but must not override a name
    Contacts or WhatsApp already resolved."""
    if identity and identity.get("name"):
        if identity.get("confidence") == "high":
            return identity["name"]
        if _looks_like_phone_number(fallback) or fallback == "Unknown":
            return identity["name"]
    return fallback
...
def resolve_contact_names(local: dict) -> dict:
    ...
    resolve_identity = build_identity_resolver(local)
    ...
    def with_im_name(msg):
        ...
        m = dict(msg)
        m["resolved_name"] = _pick_name(identity, fallback)
        return _attach(m, identity)

    def with_wa_name(msg):
        jid = _s(msg.get("contact_jid"))
        # @lid digits are a privacy id, not a phone — normalizing them would false-match a contact.
        identity = None if jid.endswith("@lid") else resolve_identity(jid)
        fallback = resolve_whatsapp_name(jid, _s(msg.get("partner_name")), lookup)
        m = dict(msg)
        m["resolved_name"] = _pick_name(identity, fallback)
        return _attach(m, identity)
...
def _action_age(created_at: str, now=None) -> str:
    """Age of a ledger item against `now` — the wall clock by default, an INJECTED instant when the
    caller has one (a corpus replay reads a day six weeks back; wall-clock ages would inflate every
    number in that day's prompt by the offset)."""
...
def _looks_like_raw_handle(name: str) -> bool:
    """A resolved value that is really a raw username/number, not a person's name — e.g. an email
    local-part like 'gupta49' or a slug like 'arnav_sahu': a single token (no space) carrying a digit
    or underscore. We keep it visible but DON'T let it read as a confident full name in a group label."""
    n = _s(name).strip()
    if not n or " " in n:
        return False
    return bool(re.search(r"[0-9_]", n))


def _handle_short_form(handle: str, channel: str) -> str:
    """Raw-handle fallback presentation for a group participant that didn't resolve to a contact
    name: the email local-part, or a phone formatted for display. Never a topical invention.
    @lid JIDs return '' — their digits are a privacy id, and formatting them as a phone number
    fabricates a number that doesn't exist (the fake '+12345…' bug)."""
    h = _s(handle)
    if not h:
        return ""
    if h.endswith("@lid"):
        return ""
    if "@" in h and not h.endswith((".whatsapp.net", "@g.us", "@c.us")):
        return h.split("@")[0]
    d = _digits(h.split("@")[0])
    return _format_phone_for_display(d) if d else h


def _join_participant_names(names: list, total: int) -> str:
    """iMessage-style label from participant display names. `names` are the presentable names (resolved
    preferred); `total` is the full participant count so unshown/unresolved members still count toward
    "& N others". Examples: ["Alice"]→"Alice"; ["A","B"]→"A & B"; ["A","B","C"] (total 3)→"A, B & C";
    4 participants → "A, B & 2 others"."""
    shown = names[:3]
    others = total - len(shown)
    if others > 0:
        head = names[:2]
        n = total - len(head)
        return f"{', '.join(head)} & {n} {'other' if n == 1 else 'others'}"
    if len(shown) == 1:
        return shown[0]
    if len(shown) == 2:
        return f"{shown[0]} & {shown[1]}"
    return f"{', '.join(shown[:-1])} & {shown[-1]}"


def _group_display_label(group_name: str, participant_handles: list, channel: str, lookup: dict,
                         identity=None) -> str:
    """The bold NAME a group chat surfaces to the LLM. Priority:
      1. the user-set group name (group_name) — used VERBATIM;
      2. an iMessage-style label from resolved participant NAMES ("Alice, Bob & 2 others");
      3. raw-handle short forms if nothing resolves.
    Returns '' when there's nothing to build from (backward-compat group keyed by sender, no
    participants) so the caller keeps today's sender-derived name.

    `identity` is `build_identity_resolver(local)` — the SAME resolver that names 1:1 threads, so a
    participant Sotto knows from the graph is named here too. It is optional only so the many
    callers that legitimately have no graph (tests, a Contacts-only path) still work; production
    callers pass it."""
    gn = _s(group_name).strip()
    if gn:
        return gn
    handles = [_s(h) for h in (participant_handles or []) if _s(h)]
    if not handles:
        return ""
    resolved, raw = [], []
    for h in handles:
        nm = (resolve_imessage_name(h, lookup) if channel == "imessage"
              else resolve_whatsapp_name(h, "", lookup))
        # @lid digits are a privacy id, not a phone — resolving them would false-match a contact,
        # the same carve-out the 1:1 WhatsApp path makes.
        if identity is not None and not h.endswith("@lid"):
            nm = _pick_name(identity(h), nm)
        if nm and nm != "Unknown" and not _looks_like_phone_number(nm) and not _looks_like_raw_handle(nm):
            resolved.append(nm)
        else:
            raw.append(_handle_short_form(h, channel))
    names = resolved or [r for r in raw if r]
    if not names:
        return ""
    return _join_participant_names(names, len(handles))


def _group_sender_display(m, channel, lookup, identity=None) -> str:
    """Display name for the SENDER of one inbound group message, so the renderer can attribute each
    group line ('[THEY SENT — Sarah Chen]') instead of leaving every group message anonymous — the
    gap that made the LLM GUESS who said what in a group. iMessage carries the sender in `handle`;
    WhatsApp group messages carry it in `sender_jid` (Bridge ≥ the ZGROUPMEMBER join) — absent that,
    return '' and the line stays unattributed (never guessed). @lid / unresolvable senders also
    return '' rather than a fabricated number. `identity` is the shared resolver — the sender of a
    group line is named by the same rule as the group's label and as their own 1:1 thread."""
    if channel == "imessage":
        h = _s(m.get("handle"))
        if not h:
            return ""
        nm = resolve_imessage_name(h, lookup)
    else:
        h = sj = _s(m.get("sender_jid"))
        if not sj:
            return ""
        if sj.endswith("@lid"):
            return lookup.get(sj) or ""   # a privacy id resolves by exact match or not at all
        nm = resolve_whatsapp_name(sj, "", lookup)
...
def _group_messages_into_threads(messages, channel, lookup=None, identity=None):
    """Port of groupIntoThreads: one thread per conversation, ordered by time. Carries is_group_chat
    (True if ANY message is a group), confidence/canonical_id (for the known-person rescue), and
    last_unreplied_ask (computed over non-system messages). `lookup` is the contact name index
    (build_contact_lookup over local.contacts) — REQUIRED for iMessage, whose handles are raw phone
    numbers the Bridge does not pre-resolve; without it names fall back to bare digits and the LLM
    mixes them up.

    Group keying: a message with a non-null `chat_guid` (the Bridge's stable group id) keys its thread
    by ("group", chat_guid) so EVERY sender in that group collapses into ONE thread — otherwise a group
    is split per-sender and the LLM re-merges and re-invents a topical name. 1:1 messages (no chat_guid)
    keep the sender-keyed behavior. BACKWARD-COMPATIBLE: when chat_guid/group_name/group_participants are
    absent (pre-rebuild Bridge), this falls back to today's sender keying. Group threads also carry
    group_name, group_participants (union of handles seen), and a computed display_label used as the
    bold NAME (never a conversation-topic invention)."""
    lookup = lookup or {}
    threads = {}
    for m in messages:
        if channel == "imessage":
        ...
        chat_guid = _s(m.get("chat_guid"))
        if (m.get("is_group_chat") or chat_guid) and not m.get("is_from_me"):
            sender = _group_sender_display(m, channel, lookup, identity)
            if sender:
                m = dict(m)
                m["sender_name"] = sender
        # "Unknown" is the resolver's sentinel (e.g. unresolvable @lid senders) — keying on it would
        # collapse every unresolved sender into ONE thread mixing different people's conversations,
        # so an unknown-name 1:1 keys by its identifier instead.
        key = ("group", chat_guid) if chat_guid else ((name if name != "Unknown" else "") or ident).lower()
        t = threads.setdefault(key, {"name": name or ident, "identifier": ident,
                                     ...
                                     "group_name": None, "group_participants": []})
        if m.get("is_group_chat") or chat_guid:
            t["is_group_chat"] = True            # group if ANY message is a group (thread-processing.ts:124)
        gn = _s(m.get("group_name"))
        if gn and not t["group_name"]:
            t["group_name"] = gn                 # carry the Bridge's user-set group name (first wins)
        for p in _arr(m, "group_participants"):
            ps = _s(p)
            if ps and ps not in t["group_participants"]:
                t["group_participants"].append(ps)   # union of participant handles seen
    ...
    for t in threads.values():
        ...
        if t["is_group_chat"]:
            label = _group_display_label(t["group_name"], t["group_participants"], channel,
                                         lookup, identity)
            # A group surfaces its display_label as the bold NAME (not a single sender). Fall back to
            # today's sender-derived name only when there's nothing to build a label from.
            t["display_label"] = label or t["name"]
            t["name"] = t["display_label"]
        else:
...
def group_label_key(label: str) -> str:
    """The lookup form of a group's display label — lowercased, whitespace collapsed. A label is a
    HUMAN name for a group, so it is matched loosely; it is never an identity (see
    `group_identity_index`)."""
    return " ".join(_s(label).split()).lower()


def group_id_key(value: str) -> str:
    """The comparison form of a group's platform id — trimmed and lowercased. iMessage's `chat_guid`
    and WhatsApp's group JID (`…@g.us`) are both opaque strings the model copies, so case/whitespace
    drift in the copy must not make it a different group."""
    return _s(value).strip().lower()


def group_identity_index(local: dict) -> dict:
    """The group threads of a read_local snapshot, indexed BOTH ways:
        {"by_id":    {group_id_key(chat_guid): (channel, display_label)},
         "by_label": {(channel, group_label_key(label)): (chat_guid, display_label)}}

    A group is the ONE counterpart the pipeline gives no per-person identifier, so its ledger
    identity used to fall back to `name:<whatever the extractor called the group that day>`. Both
    platforms DO give a group a stable id — iMessage's `chat_guid`, WhatsApp's `…@g.us` group JID,
    carried in LocalData under the same field — and `by_id` is what makes an id the model copied
    SOURCE-VERIFIED: only an id this snapshot actually contains may anchor a loop. `by_label` is the
    rare fallback for a group ask that arrives with no id at all. Built with the SAME
    `_group_messages_into_threads` the renderer uses — one grouping implementation, so the id the
    ledger anchors on is the id the model was shown."""
    lookup = build_contact_lookup(_arr(local, "contacts"))
    identity = build_identity_resolver(local)
    by_id, by_label = {}, {}
    for channel in ("imessage", "whatsapp"):
        for t in _group_messages_into_threads(_arr(local, channel), channel, lookup, identity):
            guid = _s(t.get("chat_guid"))
            if not (t.get("is_group_chat") and guid):
                continue
            label = _s(t.get("display_label")) or _s(t.get("name"))
            by_id.setdefault(group_id_key(guid), (channel, label))
            for candidate in (t.get("display_label"), t.get("group_name"), t.get("name")):
                lk = group_label_key(_s(candidate))
                if lk:
                    by_label.setdefault((channel, lk), (guid, label))
...
def _format_thread_as_text(thread, channel) -> str:
    ...
    elif thread.get("is_group_chat") and _s(thread.get("chat_guid")):
        # A group still has no deep link (hence no `identifier:` and no tap marker), but it DOES
        # have a stable id — and a loop's identity must not rest on whatever the model called the
        # group that day. Shown under its own key so it can never be mistaken for a tap target.
        lines.append(f"group_id: {_s(thread.get('chat_guid'))} | channel: {channel}")
    ...
    for m in real[-10:]:
        if m.get("is_from_me"):
            direction = "[USER SENT]"
        elif thread.get("is_group_chat") and _s(m.get("sender_name")):
            # Attribute each group line to its resolved sender so the LLM never guesses who said
            # what in a group. Lines with no resolved sender stay a bare [THEY SENT].
...
def _trim_email(e, lookup: dict | None = None) -> dict:
    ...
    # Reconcile the sender with Apple Contacts (the single biggest cross-channel identity gap):
    # without this, the person the user knows as "Sarah Chen" on iMessage surfaces under whatever
...
def _format_emails(emails) -> str:
    ...
    def fmt(e):
        ...
        if e["isSent"]:
            addr = f"To: {e['to']}\nFrom: {e['from']}"
        elif e.get("resolvedName"):
            # Contacts is authoritative for the sender's NAME — present the same name this person
            # carries on iMessage/WhatsApp/calendar so the LLM sees one identity across channels.
            addr = f"From: {e['resolvedName']} <{e['senderEmail']}>\nSenderEmail: {e['senderEmail']}"
        else:
            addr = f"From: {e['from']}\nSenderEmail: {e['senderEmail']}"
    ...
    if archived:
        lines = "\n".join(f"- {e['subject'] or '(no subject)'} "
                          f"(from: {(e.get('resolvedName') + ' <' + e['senderEmail'] + '>') if e.get('resolvedName') else e['from']})"
    ...
    for day, evs in by_day.items():
        ...
        for e in evs:
            ...
            for a in atts:
                ...
                # Contacts name first (same identity as messages/email), then the event's own
                # displayName, then the address local-part — never an invented fuller form.
                name = (lookup.get(email) if email else "") or a.get("displayName") \
...
def _format_attendee_research(inputs) -> str:
    """Render the researched attendee briefs (port of the meeting-research lines the Mac backend
    weaves into meeting_prep actions). Each entry is {email, title, company, relevance[], summary}
    plus the recency-sweep fields {recent_activity[{when,what,source_url}], personal[]} from
    research_attendees.py's Pass B (see research-prompt.md); here we only format it into the
    meeting-prep context the FLEX prompt reads."""
    ...
    for r in research:
        ...
        company_summary = _s(r.get("company_summary")).strip()
        if company_summary:   # person → company fallback: the company is researchable even
            block.append(f"  Company: {company_summary}")   # when the person isn't
        for bullet in _arr(r, "relevance"):
            b = _s(bullet).strip()
            if b:
                block.append(f"  · {b}")
        for it in _arr(r, "recent_activity"):
            what = _s(it.get("what")).strip() if isinstance(it, dict) else ""
            url = _s(it.get("source_url")).strip() if isinstance(it, dict) else ""
            when = _s(it.get("when")).strip() if isinstance(it, dict) else ""
            if what and url:
                block.append(f"  Recent: {what}" + (f" ({when})" if when else "") + f" — {url}")
        for p in _arr(r, "personal"):
            p = _s(p).strip()
            if p:
                block.append(f"  Personal: {p}")
        lines.append("\n".join(block))
    return ("## Attendee Research (PRE-COMPUTED — external people in today's meetings)\n"
            ...
            "company does) and background (prior roles, funding stage), and inform the action's prose. `Company:`\n"
            "lines describe the attendee's company when the person themselves has no public profile — use them\n"
            "(\"Cobalt Research does X; their role isn't public\") instead of writing \"no background found\". `Recent:`\n"
            "lines are dated public activity from the last ~90 days with their source URLs — the freshest, most\n"
            "specific material; prefer it over generic bio facts. `Personal:` lines are public personal texture\n"
            "(use at most one, lightly). If NONE of these lines carry anything concrete for a person,\n"
            "say so plainly and suggest nothing — never pad with generic process points (\"ask for introductions\",\n"
            "\"understand the agenda\"). Do NOT narrate this in the brief and do NOT invent facts\n"
...
def _days_from(ts, today):
    """Whole days from `today` to the day named by `ts` — negative for the past, None when the
    timestamp doesn't parse. Day granularity on purpose: the Bridge sends local wall-clock strings
    and `today` comes from the brief's instant, so comparing anything finer than the date invents
    a timezone that neither side agreed on."""
    d = _parse_ts(_date_only(ts))
    return (d.date() - today).days if d else None


def _reminder_when(r, today) -> str:
    """The parenthetical for one reminder — the whole point of the reminders section reading as
    pressure rather than as a list. Four shapes:

        (due 3 days ago)   overdue — the Bridge only started sending these; they must LOOK overdue
        (due today)        today's pressure
        (due 2026-08-12)   ahead of you — a plain date, nothing urgent implied
        (no date, written Monday)   undated, but you wrote it this week
    """
    due = _s(r.get("due_date")).strip()
    if due:
        days = _days_from(due, today)
        if days is None:
            return f"due {due}"  # unparseable — pass the raw string through rather than drop it
        if days < 0:
            n = -days
            return f"due {n} day{'s' if n != 1 else ''} ago"
        return "due today" if days == 0 else f"due {_date_only(due)}"
    written = _s(r.get("created_date")).strip()
    days = _days_from(written, today) if written else None
    if days is None or days > 0:
        return "no date"  # older Bridge build (no created_date) or a nonsense future creation date
    if days == 0:
        return "no date, written today"
    if days == -1:
        return "no date, written yesterday"
    if days > -7:
        return f"no date, written {_parse_ts(_date_only(written)).strftime('%A')}"
    return f"no date, written {_date_only(written)}"  # ≥7d back: a weekday name would be ambiguous


def _format_reminders(reminders, status=None, now=None) -> str:
    """Reminders — the last 7 days and the next 3.

    THE RULE, the same sentence the Bridge reader carries: **Sotto sees the last 7 days and the
    next 3 — a reminder counts if it's due in that span, or if you wrote it in the last 7 days.**
    Overdue and undated reminders reach this formatter now (they never used to), so each one is
    rendered relative to today instead of as a bare date the model has to do arithmetic on.

    `now` is the SAME injected instant the rest of the prompt reasons about — compose_brief passes
    `_brief_now(inputs)`, so a corpus replay of a day six weeks back measures "overdue" against that
    day, not against the wall clock. There is no second clock here; never add one.
    """
    if not reminders:
    ...
    return "\n".join(f"- {_s(r.get('title'))} ({_reminder_when(r, today)})" for r in reminders)
...
def _format_deferred_unread(local) -> str:
    ...
    if wa:
        sections.append("**WhatsApp (chats with unread)**\n" + "\n".join(
            ...
            for m in wa))
    def _em_from(e):
        addr = _s(e.get("senderEmail")).lower().strip()
        nm = lookup.get(addr) if addr else ""
        return f"{nm} <{addr}>" if nm else _s(e.get("from"))

    if em:
        sections.append("**Gmail (unread from known senders)**\n" + "\n".join(
            f"- {_em_from(e)} — \"{e.get('subject')}\" ({e.get('daysOld')}d old)\n  {e.get('snippet')}\n  Thread: {e.get('threadId')}"
            for e in em))
    return ("### Deferred Items (user saw these but hasn't acted)\n"
            ...
            "- Reference age when relevant (\"from a few days back\").\n"
...
def _format_past_commitments(local) -> str:
    ...
    return ("### Commitment History for Key People (historical context only)\n"
            "Do NOT duplicate any item already present in \"Open Commitments (ACTION LEDGER from previous briefs)\". "
...
def _format_action_ledger(local, now=None) -> str:
    ...
    for a in active:
        ...
        lines.append(f"- {a.get('action_type')} ({age}){_chase_note(a)}: "
                     ...
                     + (f" — {ask}" if ask else ""))
    body = "\n".join(lines)
    # The header states the SAME one-sentence contract the extraction prompt's TRUST BOUNDARIES do:
    # every entry below is still open (replied/passed/expired ones were resolved in code before this
    # list was built) — and only the URGENT ones earn a line. The brief decides; it does not inventory.
...
def _format_reconciliation(local, brief_type) -> str:
    ...
    return ("## Evening Accountability (morning commitments — did you follow through?)\n"
            ...
            "- If still open → list in the \"Still Pending\" section (defined in Time Awareness — one matter-of-fact line each)\n"
...
def _format_escalation_signals(signals) -> str:
    ...
    return ("## Cross-Channel Escalation (PRE-COMPUTED — trust these signals)\n"
            "These contacts contacted the user via multiple channels within 48 hours.\n"
            "Use this as a priority signal.\n\n"
            f"{body}\n\n"
            "Rules:\n"
