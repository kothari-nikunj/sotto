...
def resolve_contact_names(local: dict) -> dict:
    ...
    The canonical resolver is fed BOTH the knowledge-graph contact_index (medium confidence — LLM/
    graph-derived identifiers) AND an index seeded from Apple Contacts (high confidence, listed last
    so a Contacts identity wins any identifier collision)."""
    ...
    def _pick_name(identity, fallback):
        ...
        # identity (knowledge graph) names it ONLY when the fallback isn't a real name — a graph
        # match on an exact identifier beats showing the user a raw phone number, but must not
        # override a name Contacts/WhatsApp already resolved.
...
def _group_display_label(group_name: str, participant_handles: list, channel: str, lookup: dict) -> str:
...
def _group_sender_display(m, channel, lookup) -> str:
    ...
    return '' rather than a fabricated number."""
    if channel == "imessage":
        h = _s(m.get("handle"))
        if not h:
            return ""
...
def _group_messages_into_threads(messages, channel, lookup=None):
    ...
    for m in messages:
        ...
        if (m.get("is_group_chat") or chat_guid) and not m.get("is_from_me"):
            sender = _group_sender_display(m, channel, lookup)
            if sender:
                m = dict(m)
    ...
    for t in threads.values():
        ...
        if t["is_group_chat"]:
            label = _group_display_label(t["group_name"], t["group_participants"], channel, lookup)
...
def _format_attendee_research(inputs) -> str:
    ...
    plus the recency-sweep fields {recent_activity[{when,what,source_url}], personal[],
    conversation_hooks[]} from research_attendees.py's Pass B (see research-prompt.md); here we
    only format it into the meeting-prep context the FLEX prompt reads."""
    ...
    for r in research:
        ...
        for h in _arr(r, "conversation_hooks"):
            h = _s(h).strip()
            if h:
                block.append(f"  Hook: {h}")
        lines.append("\n".join(block))
    return ("## Attendee Research (PRE-COMPUTED — external people in today's meetings)\n"
            ...
            "(use at most one, lightly). `Hook:` lines are grounded conversation openers — fold the best one into\n"
            "the meeting_prep prose when it fits. If NONE of these lines carry anything concrete for a person,\n"
