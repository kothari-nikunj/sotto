...
Input (argv files or stdin JSON):
  ...
  --comms     /tmp/sotto_gmail.json         already-gathered emails — per-attendee relationship
                                            context (disambiguation + relevance), no new gather
Output (stdout): {"attendees":[{email,title,company,relevance[],summary,company_summary,
...
def _comms_by_email(comms, attendee_emails: set) -> dict:
    ...
    accepts the raw gmail file ({emails|messages:[…]} or a bare list) or a full inputs dict
    ({google:{emails:[…]}}). Returns {email: ["email <date> <subject>: <snippet>", …]} (≤3 each) —
    the real relationship, used to disambiguate WHICH person to research and to sharpen relevance."""
...
def main():
    ...
    ap.add_argument("--comms", help="already-gathered email JSON (/tmp/sotto_gmail.json — "
                                    "{emails:[…]}, a bare list, or a full inputs dict); optional, "
                    "gives the researcher the real relationship context per attendee")
    a = ap.parse_args()
