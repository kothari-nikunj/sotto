...
def _detect_escalation(local: dict, emails: list, now: datetime | None = None) -> list:
    ...
    email_by_person = {}
    for e in emails:
        name = _extract_sender_name(e.get("from"))
        if not name:
            continue
        ts = _s(e.get("date"))
        key = name.lower().strip()
...
def _build_cross_source_index(im_needs, im_handled, wa_needs, wa_handled, emails, events, missed) -> str:
    ...
    for em in emails:
        ...
        name = re.sub(r"<.*?>", "", from_).strip().strip('"') or from_
        if not name:
            continue
        e = get(name)
...
def build_prompt(template: str, inputs: dict) -> str:
    ...
    trimmed_emails = [_trim_email(e) for e in emails_raw]
    ...
    fields = {
        ...
        "calendar": _format_calendar(events),
...
def build_data_manifest(inputs: dict) -> dict:
    ...
    google = _obj(inputs, "google")
    emails = [_trim_email(e) for e in _arr(google, "emails")]
    events = _arr(google, "events")
    local = resolve_contact_names(_normalize_local(inputs))
    ...
    for e in emails:
        ...
        if tid:
            seen.add(tid)
        threads.append({"subject": e.get("subject") or "(no subject)", "from": e.get("from"),
