...
from chatfmt import to_chat  # noqa: E402  (render_chat_text delegates to the shared transformation)
...
def select_attendees_for_research(inputs: dict) -> list:
    ...
    user_email = (_s(google.get("userEmail")) or _s(os.environ.get("SOTTO_USER_EMAIL"))).lower()
...
def _unwrap_local(obj):
    ...
    silently kills headless/cron briefs). The peeling itself lives in textutil.unwrap_tool_result —
    ONE shared implementation for every read_local consumer (style_extract, continuity_resolve,
    prewarm_graph unwrap the same way, so a wrapper never zeroes just one of them)."""
...
def _normalize_local(inputs: dict) -> dict:
    ...
    # so the brief drops them exactly like /api/loops and the overview do. Replicated inline (three
    # lines) rather than imported — dashboard.py and continuity_resolve.py live behind boundaries
    # this script must not reach across.
    if not local.get("action_ledger"):
        try:
            sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            import ledger_io  # noqa: PLC0415
            entries = [e for e in ledger_io.load_active()
                       if _s(e.get("action_type")).lower().replace("-", "_")
...
def call_gemini(prompt: str, inputs: dict, system: str | None = None, schema: dict | None = None) -> str:
    ...
    generationConfig.responseSchema — see gemini._gemini_once.
...
# The triage policy the brief was WRITTEN under, embedded verbatim (extraction-prompt.md's Triage
# Discipline + Priority Levels). The critic judges against THIS bar — without it, the last model to
# touch every brief re-inflated what the extraction deliberately pruned (the additive-critic bug).
_CRITIC_TRIAGE_POLICY = """## Triage Policy (the SAME policy the brief was written under — judge against this bar, NOT against completeness)

Triage Discipline — before including each entry, the brief asks: "Would a great chief of staff interrupt for this?"
...
def _action_tap_link(action: dict, event_links: dict | None = None) -> str:
    ...
    (action_links.link_for — also the drafts.jsonl recording chokepoint). What lives here is the
    ...
    def _link(channel: str, identifier: str) -> str:
        # No message: a brief's tap link opens the thread, it never prefills a draft — so this call
        # records nothing to drafts.jsonl (record_draft skips messageless links).
...
def render_chat_text(markdown: str) -> str:
    ...
    Thin delegate to the ONE shared markdown→chat transformation, chatfmt.to_chat (markers stripped,
    headings/**bold** → WhatsApp *bold*, hrules dropped, blank runs collapsed) — kept under its old
    name for the `cb.render_chat_text` call sites and tests.
    brief_markdown stays untouched in the output for records/critic/actions."""
    return to_chat(markdown)
...
def _followup_evening_context(inputs: dict, llm) -> dict:
    ...
    try:
        ...
        f_inputs = {
            ...
            "google": google,
            "user_email": _s(google.get("userEmail")) or _s(os.environ.get("SOTTO_USER_EMAIL")),
...
def _apply_followup_commitments(fu: dict, inputs: dict) -> None:
    ...
    try:
        ...
        user_email = _s(_obj(inputs, "google").get("userEmail")) or _s(os.environ.get("SOTTO_USER_EMAIL"))
...
def _append_still_open(out: dict, open_ledger: list) -> dict:
    ...
    try:
        ...
        lines = []
        for e in missed:
            age = _action_age(_s(e.get("created_at")))
        ...
        out["brief_markdown"] = (md[:m.start()] + block + md[m.start():]) if m else (md.rstrip() + "\n\n" + block)
...
def compose(inputs: dict, llm=call_gemini, critic: bool = False) -> dict:
    ...
    out = _append_still_open(out, open_ledger)
