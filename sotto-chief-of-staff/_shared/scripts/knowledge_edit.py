...
Output: one JSON object on stdout — {"ok": true, "slug", "fact_count"} for fact ops,
{"ok": true, "anchor_key", "status"} for loop — or {"ok": false, "error"} + exit 2 on failure.
"""
...
def run(argv: list) -> dict:
    ...
    ap.add_argument("--op", required=True, choices=("correct", "archive", "add", "loop"))
