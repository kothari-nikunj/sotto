#!/usr/bin/env python3
"""
followup_cron.py — windowing + run-marker + silence gate for the (retired) standalone followup cron.

The followup extraction now runs INSIDE the evening brief (SOTTO_FOLLOWUP_MERGED, default 1 — see
followup_merged() below), so the standalone 16:45 evening cron is RETIRED: an already-registered cron
becomes a no-op (--silent-check prints "silent" unconditionally). The followup skill itself still
runs on-demand, and never consults this helper on that path. Set SOTTO_FOLLOWUP_MERGED=0 to re-enable
the standalone cron; everything below describes that escape-hatch path.
...
Env: SOTTO_DATA (state dir), SOTTO_FOLLOWUP_DEFAULT_HOURS (first-run window, default 36),
     ...
     SOTTO_FOLLOWUP_MERGED (default 1 — the evening brief carries the followup, so the standalone
     cron's --silent-check prints "silent" unconditionally; set 0 to re-enable the standalone cron).
"""
...
def followup_merged() -> bool:
    """Roadmap Sprint 0 #4: the followup extraction now runs INSIDE the evening brief, so the
    standalone 16:45 cron is retired. Default ON — an already-registered cron becomes a no-op (its
    --silent-check always says 'silent') even before it's deregistered. Set SOTTO_FOLLOWUP_MERGED=0
    to run the standalone cron again. The on-demand skill path never consults this."""
    return (os.environ.get("SOTTO_FOLLOWUP_MERGED", "1").strip().lower()
...
def main():
    ...
    if a.stamp:
        ...
        return
    if a.silent_check:
        # Merged mode (default): the evening brief carries the followup now — the standalone cron is
        # unconditionally silent, whatever the file says (or whether it exists). This neuters a
        # still-registered 16:45 cron the moment the merge ships, before deregistration.
        if followup_merged():
            print("silent")
            return
