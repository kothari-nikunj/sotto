...
import textutil      # noqa: E402  — the owners of the helpers exercised below
...
def test_granola_gather_warnings_mark_source_unavailable():
    # gather_granola.py embeds "warnings" in its output on any failure — the brief must render the
    # honest Data Source Availability line instead of treating a broken gather as a quiet week.
    inputs = {"type": "morning", "google": {"events": []}, "local": {},
              "granola": {"meetings": [], "warnings": ["Granola auth failed — reconnect on /setup"]}}
    local = cb._normalize_local(inputs)
    assert local["_source_availability"]["granola"] == "unavailable"
    prompt = cb.build_prompt(cb._load_prompt(), inputs)
    assert "Data Source Availability" in prompt
    assert "Meeting Notes (Granola)" in prompt


def test_granola_warnings_merge_with_bridge_source_status():
    # The Bridge's source_status mapping must survive alongside the granola-warning flag.
    inputs = {"type": "morning", "google": {"events": []},
              "local": {"source_status": {"imessage": "ok", "whatsapp": "needs_fda"}},
              "granola": {"meetings": [], "warnings": ["mcp lane failed: boom"]}}
    local = cb._normalize_local(inputs)
    assert local["_source_availability"]["whatsapp"] == "unavailable"
    assert local["_source_availability"]["imessage"] == "available"
    assert local["_source_availability"]["granola"] == "unavailable"


def test_granola_clean_gather_leaves_availability_alone():
    inputs = {"type": "morning", "google": {"events": []}, "local": {},
              "granola": {"meetings": [{"title": "Sync"}]}}
...
def test_looks_like_phone_number():
    assert textutil._looks_like_phone_number("")           # empty
    assert textutil._looks_like_phone_number("+15551234567")  # starts with +
    assert textutil._looks_like_phone_number("(555) 123-4567")  # starts with (
    assert textutil._looks_like_phone_number("262966")     # shortcode, all digits
    assert not textutil._looks_like_phone_number("Sarah Chen")
    assert not textutil._looks_like_phone_number("Devon")
...
def test_strict_names_match():
    ...
    assert textutil._names_match("Jake Helberg", "Jake Helberg")
    assert textutil._names_match("Jake H", "Jake Helberg")
    assert textutil._names_match("Jake Hel", "Jake Helberg")
    # First-name-only must NOT match (the loose substring bug this replaced).
    assert not textutil._names_match("Marcus", "Marcus Wallace")
    assert not textutil._names_match("Sam", "Samantha Lee")
    # Diacritics normalized.
    assert textutil._names_match("Tomás García", "Tomas Garcia")


def test_is_likely_automated():
    assert textutil._is_likely_automated("no-reply@stripe.com")
...
def test_short_ack_keeps_thread_in_needs_response():
    ...
    threads = render_local._group_messages_into_threads([
    ...
    assert render_local._thread_needs_response(threads[0]) is True
    # A substantive reply clears it.
    threads2 = render_local._group_messages_into_threads([
        {"handle": "+1", "is_from_me": False, "timestamp": "2026-06-24 09:00:00",
         "text": "Can you send me the deck please?"},
...
def test_group_chat_guid_collapses_senders_into_one_thread():
    ...
    lookup = render_local.build_contact_lookup([
    ...
    header = render_local._format_thread_as_text(t, "imessage").splitlines()[0]
    assert header == "### Weekend Crew [GROUP - no deep link]"
    # …and the group's own stable id, under its own key: a group has no deep link (so no
    # `identifier:` line and no tap marker), but it DOES have an identity the ledger can anchor on.
    assert render_local._format_thread_as_text(t, "imessage").splitlines()[1] == \
        "group_id: grp-1 | channel: imessage"


def test_group_identity_index_maps_ids_and_labels_both_ways():
    """A group's identity is its platform id — iMessage's chat_guid, WhatsApp's `…@g.us` JID. The
    index is what makes an id the model copied SOURCE-VERIFIED, with the label as the rare fallback."""
    local = {"contacts": [{"name": "Alice Ng", "phones": ["+14150000001"]}],
             "imessage": [{"handle": "+14150000001", "is_from_me": False,
                           "timestamp": "2026-06-24 09:00:00", "text": "hi", "is_group_chat": True,
                           "chat_guid": "grp-1", "group_name": "Weekend Crew",
                           "group_participants": ["+14150000001"]}],
             "whatsapp": [{"contact_jid": "14150000002@s.whatsapp.net", "is_from_me": False,
                           "timestamp": "2026-06-24 09:00:00", "text": "yo", "is_group_chat": True,
                           "chat_guid": "120363111@g.us", "group_name": "Weekend Hikers",
                           "group_participants": ["14150000002@s.whatsapp.net"]}]}
    idx = render_local.group_identity_index(local)
    assert idx["by_id"]["grp-1"] == ("imessage", "Weekend Crew")
    assert idx["by_id"]["120363111@g.us"] == ("whatsapp", "Weekend Hikers")
    assert idx["by_label"][("imessage", "weekend crew")] == ("grp-1", "Weekend Crew")
    assert idx["by_label"][("whatsapp", "weekend hikers")] == ("120363111@g.us", "Weekend Hikers")
    # a 1:1 thread is not a group and never enters the index
    assert render_local.group_identity_index(
        {"imessage": [{"handle": "+1", "is_from_me": False, "timestamp": "t", "text": "x",
                       "is_group_chat": False}]}) == {"by_id": {}, "by_label": {}}
...
def test_whatsapp_named_group_uses_subject_verbatim():
    ...
    header = render_local._format_thread_as_text(t, "whatsapp").splitlines()[0]
...
def test_escalation_detection_cross_channel():
    ...
    assert r and r[0]["name"] == "Sarah Chen"   # display-cased, not the lowercased dedup key
...
def test_research_no_provider_marks_the_source_unavailable_in_the_brief():
    """research_attendees.py embeds a warning when the search ladder had no provider to ask. That
    reached stderr only, so a brief with zero researched attendees read as "nobody worth researching
    today" — a silent lie about a broken source. It now travels the SAME road a failed Granola
    gather does: an unavailable source the brief states."""
    inputs = {"type": "morning", "google": {"events": []}, "local": {},
              "attendee_research": {"attendees": [],
                                    "warnings": ["no search provider connected — set EXA_API_KEY, "
                                                 "PARALLEL_API_KEY, or GOOGLE_AI_API_KEY"]}}
    # compose() flattens the envelope for the renderer and keeps the warning as a private input…
    local = cb._normalize_local({**inputs, "_research_warnings": inputs["attendee_research"]["warnings"]})
    assert local["_source_availability"]["attendee_research"] == "unavailable"
    # …and the prompt says so in plain words, with a label a reader understands.
    prompt = cb.build_prompt(cb._load_prompt(),
                             {**inputs, "attendee_research": [],
                              "_research_warnings": inputs["attendee_research"]["warnings"]})
    assert "Data Source Availability" in prompt
    assert "Attendee Research (web search)" in prompt


def test_research_that_ran_leaves_availability_alone():
    inputs = {"type": "morning", "google": {"events": []}, "local": {},
