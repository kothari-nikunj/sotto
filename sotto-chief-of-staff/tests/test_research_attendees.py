...
def _local_date(days_ago=0):
    return (ra.cb._now_local(ra.cb.configured_tz()) - timedelta(days=days_ago)).strftime("%Y-%m-%d")
...
def test_no_key_degrade_is_narrated(tmp_path, monkeypatch, capsys):
    # {"attendees":[]} with zero signal read as "research ran and found nothing" — the no-key
    # degrade must say so on stderr so the agent narrates honestly.
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
    monkeypatch.delenv("SOTTO_LLM_STUB", raising=False)
    monkeypatch.delenv("GOOGLE_AI_API_KEY", raising=False)
    assert ra.research([{"name": "A", "email": "a@b.com"}], "") == {"attendees": []}
    assert "[research] skipped — GOOGLE_AI_API_KEY not set" in capsys.readouterr().err
