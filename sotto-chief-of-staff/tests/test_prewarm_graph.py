...
def test_stubs_only_by_default(tmp_path, monkeypatch):
    ...
    dhruv = open(os.path.join(people, "dhruv.md")).read()
    ...
    assert os.path.exists(os.path.join(people, "sarah.md"))
...
def test_opt_in_research_writes_low_conf_facts(tmp_path, monkeypatch):
    ...
    dhruv = open(os.path.join(str(tmp_path), "knowledge", "people", "dhruv.md")).read()
