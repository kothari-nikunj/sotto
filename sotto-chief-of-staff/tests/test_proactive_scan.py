...
def test_snooze_suppresses_every_nudge_like_quiet_hours():
    now = _at(10)
    today = now.strftime("%Y-%m-%d")
    cont = [{"id": "c1", "title": "Send the LOI", "deadline": today}]
    local = {"contacts": [{"name": "Jordan", "birthday": now.strftime("%m-%d")}]}
    assert ps.scan([], cont, local, "me@x.com", now)["nudges"]            # would fire…
    until = (now + timedelta(hours=3)).strftime("%Y-%m-%dT%H:%M")
    out = ps.scan([], cont, local, "me@x.com", now, snooze_until=until)   # …but the user asked for quiet
    assert out["nudges"] == [] and out["quiet"] is True
    assert until in out["reason"]
    # An expired stamp is inert (and a garbled one can never silence the watcher forever).
    past = (now - timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M")
    assert ps.scan([], cont, local, "me@x.com", now, snooze_until=past)["nudges"]
    assert ps.scan([], cont, local, "me@x.com", now, snooze_until="garbage")["nudges"]


def test_main_reads_the_snooze_from_preferences_and_burns_no_dedup_state(tmp_path, monkeypatch, capsys):
    """End-to-end wiring: main reads explicit.nudge_snooze_until (what the sotto-feedback verbs
    write) — and because the run reports `quiet`, nothing is marked as nudged, so a birthday held
    during the snooze can still surface after it lifts."""
    import json
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
    monkeypatch.setenv("SOTTO_TIMEZONE", "+00:00")
    monkeypatch.setenv("SOTTO_QUIET_START", "0")                 # start == end → quiet never
    ...
    assert ps._load_state(date) == set()                          # nothing burned by the snooze
