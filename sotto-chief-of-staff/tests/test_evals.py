...
def test_edge_group_names_are_not_topical_inventions():
    """Group chats render their group_name VERBATIM (named) or an iMessage-style participant label
    (unnamed) as the bold header — never a topic invented from the conversation — and the group reply
    action never gets a deep link."""
    r, _ = _evaluate("edge_day")
    for chk in (ev.chk_edge_group_names_not_invented, ev.chk_edge_group_no_deeplink,
                ev.chk_edge_group_backward_compat):
        cname, ok, detail = chk(r)
        assert ok, f"{cname}: {detail}"
