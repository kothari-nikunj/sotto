...
ku = _load("ku_identity", "_shared/knowledge/knowledge_update.py")
...
def test_push_name_beats_lossy_last10_for_foreign_numbers():
    # +81 80-1234-5678 (Japan) shares its trailing 10 digits with a US contact +1 801-234-5678.
    # The lossy last-10 fallback used to run BEFORE partner_name, renaming Kenji "Dan Utah".
    lookup = rl.build_contact_lookup([{"name": "Dan Utah", "phones": ["+1 801-234-5678"]}])
    assert rl.resolve_whatsapp_name("818012345678@s.whatsapp.net", "Kenji Sato", lookup) == "Kenji Sato"
    # Exact-form Contacts matches still beat the push name (Contacts stays authoritative there).
    assert rl.resolve_whatsapp_name("18012345678@s.whatsapp.net", "danny boy", lookup) == "Dan Utah"
    # No push name → the last-10 fallback still names the number rather than showing bare digits.
    assert rl.resolve_whatsapp_name("818012345678@s.whatsapp.net", "", lookup) == "Dan Utah"


def test_unknown_lid_senders_do_not_collapse_into_one_thread():
    # Two DIFFERENT unresolved @lid senders both resolve to the "Unknown" sentinel — keying
    # 1:1 threads by name used to merge them into ONE thread mixing two humans' conversations.
    msgs = [
        {"contact_jid": "111@lid", "text": "hey, can you call me?", "is_from_me": False,
         "timestamp": "2026-07-01 09:00:00"},
        {"contact_jid": "222@lid", "text": "totally unrelated chat", "is_from_me": False,
         "timestamp": "2026-07-01 09:05:00"},
    ]
    threads = rl._group_messages_into_threads(msgs, "whatsapp", LOOKUP)
    assert len(threads) == 2
    assert {t["identifier"] for t in threads} == {"111@lid", "222@lid"}
...
def test_knowledge_query_calendar_exempts_todays_cast_from_mtime_gate(tmp_path, monkeypatch):
    # A person whose graph file is older than --relevant-days but who is on today's calendar must
    # still pack; a stale person NOT on the calendar stays gated.
    import subprocess
    import time as _time
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
    ku.apply({"person_updates": [
        {"person_name": "Sarah Chen", "identifier": "sarah@acme.com",
         "facts": [{"fact": "CTO at Acme Corp", "memory_type": "context", "confidence": 0.9}]},
        {"person_name": "Old Pal", "identifier": "pal@x.com",
         "facts": [{"fact": "Went quiet", "memory_type": "context", "confidence": 0.9}]},
    ]})
    old = _time.time() - 21 * 86400          # both files last rewritten three weeks ago
    for name in os.listdir(kg.people_dir()):
        os.utime(os.path.join(kg.people_dir(), name), (old, old))
    cal = tmp_path / "cal.json"
    cal.write_text(json.dumps([{"summary": "Sync", "attendees": [{"email": "Sarah@Acme.com"}]}]))
    env = dict(os.environ, SOTTO_DATA=str(tmp_path))
    proc = subprocess.run([sys.executable,
                           os.path.join(ROOT, "_shared", "knowledge", "knowledge_query.py"),
                           "--relevant-days", "7", "--calendar", str(cal)],
                          capture_output=True, text=True, env=env)
    out = json.loads(proc.stdout)
    cid = {e["identifiers"][0]: e["canonical_id"] for e in out["contact_index"]}
    assert cid["sarah@acme.com"] in out["person_knowledge"]      # on the calendar → packed
    assert cid["pal@x.com"] not in out["person_knowledge"]       # stale + absent → still gated
    # Without --calendar both stay gated (the old behavior is intact).
    proc2 = subprocess.run([sys.executable,
                            os.path.join(ROOT, "_shared", "knowledge", "knowledge_query.py"),
                            "--relevant-days", "7"], capture_output=True, text=True, env=env)
    assert json.loads(proc2.stdout)["person_knowledge"] == {}
...
# ── one resolver, both paths (the "raw phone numbers in a group label" regression) ───────────────

_GRAPH_ONLY = {
    "contacts": [{"name": "Sarah Chen", "phones": ["+1 (415) 555-1234"], "emails": ["sarah@acme.com"]}],
    # Two people the GRAPH knows (they email him) who are NOT saved in Apple Contacts.
    "contact_index": [
        {"canonical_id": "c1", "display_name": "Dev Patel",
         "identifiers": ["+14042474568", "dev@acme.test"], "confidence": "medium"},
        {"canonical_id": "c2", "display_name": "Priya Rao",
         "identifiers": ["+14086663190", "priya@acme.test"], "confidence": "medium"},
    ],
    "imessage": [
        {"handle": "+14042474568", "chat_guid": "iMessage;+;chat9", "is_group_chat": True,
         "group_participants": ["+14042474568", "+14086663190"],
         "text": "lunch Thursday or Friday? come see the new office",
         "timestamp": "2026-08-11 09:00:00", "is_from_me": False},
    ],
    "whatsapp": [], "calls": [], "whatsapp_calls": [],
    "deferred_unread_imessage": [], "deferred_unread_whatsapp": [],
}


def test_a_group_label_uses_the_same_resolver_the_1to1_path_uses():
    """THE BUG, from a real brief: a 2-person group rendered as
    '+1 (404) 247-4568 & +1 (408) 666-3190' while both people were named correctly in their own 1:1
    threads. Cause: the 1:1 path resolved through the graph+Contacts resolver, the GROUP label
    resolved through Apple Contacts alone. One job, two implementations, two answers."""
    local = rl.resolve_contact_names(_GRAPH_ONLY)
    assert local["imessage"][0]["resolved_name"] == "Dev Patel"   # the 1:1 path always worked

    lookup = rl.build_contact_lookup(_GRAPH_ONLY["contacts"])
    identity = rl.build_identity_resolver(_GRAPH_ONLY)
    t = rl._group_messages_into_threads(local["imessage"], "imessage", lookup, identity)[0]
    assert t["display_label"] == "Dev Patel & Priya Rao"
    assert "404" not in t["display_label"] and "408" not in t["display_label"]
    # …and each inbound group line is attributed by the same rule.
    assert t["messages"][0]["sender_name"] == "Dev Patel"


def test_apple_contacts_still_outrank_the_graph_in_a_group_label():
    """The resolver's precedence is not bypassed by making it available here: a Contacts name is
    high-confidence and wins, so a graph nickname can never rename someone the user has saved."""
    local = dict(_GRAPH_ONLY)
    local["contact_index"] = _GRAPH_ONLY["contact_index"] + [
        {"canonical_id": "c3", "display_name": "S. Chen (work)",
         "identifiers": ["+14155551234"], "confidence": "medium"}]
    local["imessage"] = [dict(_GRAPH_ONLY["imessage"][0],
                              group_participants=["+14155551234", "+14042474568"])]
    resolved = rl.resolve_contact_names(local)
    t = rl._group_messages_into_threads(resolved["imessage"], "imessage",
                                        rl.build_contact_lookup(local["contacts"]),
                                        rl.build_identity_resolver(local))[0]
    assert t["display_label"] == "Sarah Chen & Dev Patel"


def test_a_user_set_group_name_still_wins_over_every_resolution():
    local = dict(_GRAPH_ONLY)
    local["imessage"] = [dict(_GRAPH_ONLY["imessage"][0], group_name="Offsite planning")]
    resolved = rl.resolve_contact_names(local)
    t = rl._group_messages_into_threads(resolved["imessage"], "imessage",
                                        rl.build_contact_lookup(local["contacts"]),
                                        rl.build_identity_resolver(local))[0]
    assert t["display_label"] == "Offsite planning"


def test_an_lid_participant_is_never_resolved_by_normalized_digits():
    """@lid is a WhatsApp privacy id, not a phone. The 1:1 path has always refused to normalize it;
    the group path must refuse identically, or a privacy id false-matches a real contact."""
    local = {
        "contacts": [], "whatsapp": [
            {"contact_jid": "12345678901234@lid", "chat_guid": "99@g.us", "is_group_chat": True,
             "sender_jid": "12345678901234@lid",
             "group_participants": ["12345678901234@lid", "14042474568@s.whatsapp.net"],
             "text": "hi", "timestamp": "2026-08-11 09:00:00", "is_from_me": False}],
        "contact_index": [{"canonical_id": "c9", "display_name": "Wrong Person",
                           "identifiers": ["+11234567890"], "confidence": "medium"},
                          {"canonical_id": "c1", "display_name": "Dev Patel",
                           "identifiers": ["+14042474568"], "confidence": "medium"}],
        "imessage": [], "calls": [], "whatsapp_calls": [],
        "deferred_unread_imessage": [], "deferred_unread_whatsapp": [],
    }
    resolved = rl.resolve_contact_names(local)
    t = rl._group_messages_into_threads(resolved["whatsapp"], "whatsapp",
                                        rl.build_contact_lookup([]),
                                        rl.build_identity_resolver(local))[0]
    assert "Wrong Person" not in t["display_label"]
    # The lid member stays unresolved and still counts toward the total — an unnameable participant
    # is "& 1 other", never a name borrowed from someone whose digits happened to collide.
    assert t["display_label"] == "Dev Patel & 1 other"


def test_the_resolver_is_reachable_from_every_production_thread_builder():
    """Regression on the SHAPE of the fix, not just its output: the three production callers that
    build threads must each pass an identity resolver. A future caller that forgets is the bug."""
    import triage_queue  # noqa: PLC0415
    for mod, fn in ((cb, "compose_brief"), (triage_queue, "triage_queue"), (rl, "render_local")):
