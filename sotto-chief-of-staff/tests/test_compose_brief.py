...
def test_tap_link_delegates_to_the_one_builder(monkeypatch):
    """D3: action_links.link_for is the ONLY link builder (and the drafts.jsonl chokepoint) —
    _action_tap_link picks the channel/identifier and delegates, so no URL scheme lives twice."""
    calls = []
...
def test_tap_links_record_no_drafts(tmp_path, monkeypatch):
    """Delegating must not pollute the offered-draft ledger: a brief's tap link opens a thread, it
    carries no drafted message, and record_draft skips messageless links."""
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
    cb._action_tap_link({"channel": "whatsapp", "contactIdentifier": "+15551234567"})
    cb._action_tap_link({"channel": "email", "emailReplyTo": "d@acme.com", "emailSubject": "LOI"})
...
def test_freemail_user_domain_does_not_skip_freemail_attendees(monkeypatch):
    ...
    # Headless briefs never pass --user-email, so SOTTO_USER_EMAIL is the only source of "who am I".
    # Without the env fallback here, the user is researched as an attendee of their own meeting and
    # the freemail rule above can never fire in production.
