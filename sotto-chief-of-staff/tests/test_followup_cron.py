...
def _silent_check(path, capsys, monkeypatch, merged=None):
    """Drive the --silent-check CLI path and return the single token it prints. `merged` sets
    SOTTO_FOLLOWUP_MERGED explicitly; None leaves the environment default (merged ON)."""
    if merged is None:
        monkeypatch.delenv("SOTTO_FOLLOWUP_MERGED", raising=False)
    else:
        monkeypatch.setenv("SOTTO_FOLLOWUP_MERGED", merged)
    monkeypatch.setattr(sys, "argv", ["followup_cron.py", "--silent-check", str(path)])
    fc.main()
    return capsys.readouterr().out.strip()


# --- standalone-cron behavior (SOTTO_FOLLOWUP_MERGED=0 — the pre-merge semantics) ----------------
...
def test_silent_check_missing_file_is_error(tmp_path, capsys, monkeypatch):
    ...
    assert _silent_check(tmp_path / "nope.json", capsys, monkeypatch, merged="0") == "error"


def test_silent_check_corrupt_json_is_error(tmp_path, capsys, monkeypatch):
    bad = tmp_path / "out.json"
    bad.write_text("{not valid json")
    assert _silent_check(bad, capsys, monkeypatch, merged="0") == "error"
...
def test_silent_check_valid_empty_is_silent(tmp_path, capsys, monkeypatch):
    ...
    empty.write_text(json.dumps({"followup_markdown": "Nothing to follow up on.",
                                 "commitments": [], "drafts": []}))
    assert _silent_check(empty, capsys, monkeypatch, merged="0") == "silent"
...
def test_silent_check_actionable_is_deliver(tmp_path, capsys, monkeypatch):
    ...
    assert _silent_check(out, capsys, monkeypatch, merged="0") == "deliver"


# --- merged mode (roadmap Sprint 0 #4): the evening brief carries the followup ---------------------

def test_merged_default_makes_cron_unconditionally_silent(tmp_path, capsys, monkeypatch):
    """Default (env unset) = merged: an actionable result, an empty result, and even a MISSING file
    all print 'silent' — a still-registered 16:45 cron is a no-op before it's deregistered."""
    actionable = tmp_path / "out.json"
    actionable.write_text(json.dumps({"commitments": [{"what": "send deck"}], "drafts": []}))
    assert _silent_check(actionable, capsys, monkeypatch) == "silent"
    assert _silent_check(tmp_path / "nope.json", capsys, monkeypatch) == "silent"
    # explicit =1 behaves the same
    assert _silent_check(actionable, capsys, monkeypatch, merged="1") == "silent"


def test_merged_flag_parsing():
    import os
    for val, expect in (("", True), ("1", True), ("true", True), ("yes", True),
                        ("0", False), ("false", False), ("no", False), ("off", False)):
        os.environ["SOTTO_FOLLOWUP_MERGED"] = val
        try:
            assert fc.followup_merged() is expect, (val, expect)
        finally:
            del os.environ["SOTTO_FOLLOWUP_MERGED"]
    assert fc.followup_merged() is True                    # unset → merged ON


def test_merged_mode_leaves_window_and_stamp_paths_alone(tmp_path, capsys, monkeypatch):
    # --since-hours and --stamp are untouched by the merge flag (on-demand tooling still works).
    monkeypatch.delenv("SOTTO_FOLLOWUP_MERGED", raising=False)
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
    monkeypatch.setattr(sys, "argv", ["followup_cron.py", "--stamp", "--now", "2026-07-06T16:45:00Z"])
    fc.main()
    assert capsys.readouterr().out.strip() == "stamped"
    monkeypatch.setattr(sys, "argv", ["followup_cron.py", "--now", "2026-07-07T16:45:00Z"])
    fc.main()
