...
def test_local_queue_drops_unresolved_unknown_senders():
    # An @lid sender with no push name resolves to the "Unknown" sentinel — drafting a reply to an
    # unidentified human risks messaging the wrong person, so the thread never enters the queue.
    local = {"contacts": [],
             "whatsapp": [{"contact_jid": "123456789012345@lid", "text": "are we still on?",
                           "is_from_me": False, "timestamp": "2026-06-25T09:00"}]}
    assert tq.local_queue(local, "whatsapp") == []
