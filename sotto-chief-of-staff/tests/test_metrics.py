...
thread-safe accumulation, no-op safety without a run, and one stubbed end-to-end compose run that
proves the [brief-cost] line actually gets written.
"""
...
def test_emit_writes_the_brief_cost_line(tmp_path, monkeypatch):
    """ONE output: the human [brief-cost] line. (The old brief_metrics.jsonl twin was written every
    brief and read by nothing — deleted; it returns with a dashboard cost card.)"""
    ...
    assert "date=2026-07-06" in line and "calls=1" in line and "in=1.0k" in line
    assert "extract=5.0s/1.0k/0.2k" in line and "skipped=critic,revise" in line
    # human line went to the shared brief log — the only file emit() touches
...
def test_stubbed_compose_emits_brief_cost_line(tmp_path, monkeypatch):
    ...
    # call, then emits the [brief-cost] line.
    monkeypatch.setenv("SOTTO_DATA", str(tmp_path))
