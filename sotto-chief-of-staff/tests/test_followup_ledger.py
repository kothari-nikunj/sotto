...
def _canchor(email, owner, what, family="follow_up"):
    """The content-aware anchor for a recipient commitment: contact anchor + sha(owner|what)
    suffix, so distinct commitments to the same person never collapse. `family` is the direction:
    what the user owes anchors under `follow_up`, what they're OWED under `waiting_on`."""
    return f"{family}:id:{email}:c:" + hashlib.sha256(f"{owner}|{what}".encode()).hexdigest()[:12]
...
def test_commitment_does_not_merge_with_brief_created_loop(tmp_path, monkeypatch):
    ...
    items = _ledger_items(tmp_path)
    assert len(items) == 2
    assert "follow_up:id:dana@acme.com" in items                        # the brief's loop
...
def test_other_owner_becomes_waiting_on(tmp_path, monkeypatch):
    ...
    ac.apply({"commitments": [{"meeting": "Sync", "owner": "Dana", "what": "send the contract",
                               "due": None, "to_email": "dana@acme.com"}]}, USER, NOW)
    it = _ledger_items(tmp_path)[_canchor("dana@acme.com", "Dana", "send the contract", "waiting_on")]
...
def test_default_now_uses_user_timezone(tmp_path, monkeypatch):
    ...
    res = ac.apply({"commitments": [
        {"meeting": "Sync", "owner": "you", "what": "send notes", "to_email": "dana@acme.com"}]}, USER)
    assert res["written"] == 1 and seen["tz"] == "-07:00"
    it = _ledger_items(tmp_path)[_canchor("dana@acme.com", "you", "send notes")]
    assert it["created_at"] == "2026-07-01"        # user-local date, not UTC's 2026-07-02
