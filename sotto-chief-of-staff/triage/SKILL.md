---
name: sotto-triage
description: 'Use when the user says "triage my inbox" / "let''s clear what needs me" / "process my messages" / "inbox zero" / "what do I owe people" / "clear my queue" — walk the cross-channel needs-a-reply queue (email + iMessage + WhatsApp) one item at a time: draft a reply, archive/label email, or mark a thread handled so it stops re-surfacing. This is THE way to run a triage pass — never hand-improvise a list.'
metadata:
  hermes:
    tags: [chief-of-staff, sotto, triage]
    category: productivity
    requires_toolsets: [sotto-local, google-workspace]
    requires_tools: [execute_code]
required_environment_variables:
  - name: SOTTO_DATA
    prompt: Path to the Sotto exhaust volume (e.g. /data)
    required_for: continuity (marking threads handled)
---

# Sotto — Triage (clear what needs you, across channels)

A focused working session to clear the cross-channel **needs-a-reply** queue — email AND local
(iMessage/WhatsApp). Unlike the brief (informational), triage is **action**: for each item, draft/send,
archive/label (email only), or mark it handled so tomorrow's brief doesn't renag.

## Procedure

> **Script paths:** absolute, e.g. `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/triage_queue.py"`.

1. **Gather** (deterministic — don't hand-fetch):
   - `read_local(since_hours=72)` → `/tmp/sotto_local.json` (or the staged payload).
   - `execute_code` → `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/gather_google.py" --skip-calendar` → `/tmp/sotto_gmail.json` (recent + unread email). **Host-agnostic fallback:** if it reports the CLI isn't this host's Google path (`google_api.py not found … FALLBACK …`), fetch `newer_than:1d` with the host's Gmail **MCP** tool, dump raw to `/tmp/sotto_gmail_raw.json`, then `gather_google.py --skip-calendar --from-mcp-gmail /tmp/sotto_gmail_raw.json`.
2. **Build the queue** — `execute_code` → `python3 "$HOME/.hermes/skills/sotto/_shared/scripts/triage_queue.py"` → JSON `{email[], imessage[], whatsapp[], counts}`. Each item is name-resolved with a `last_snippet`. It logs counts to `/debug/brief-log`. Walk it in priority order: **email (important/unread first) → iMessage → WhatsApp**.
3. **Per item, propose ONE action** (fast pass — no narrative):
   - **Reply** → hand to `sotto-draft-reply` (drafts in the user's voice + a one-tap send link, or true send per channel). Email/Calendar can send cloud-side; iMessage/SMS need the Bridge live (else deep link); WhatsApp is deep-link only.
   - **Declinable ask → two drafts, not one** (`sotto-draft-reply`'s no-draft rule). If the item is a request the user could plausibly say no to — time, money, an intro, a favor — the item gets `Accept:` and `Decline:` as two labeled drafts, decline second, or decline FIRST when it's clearly low-value (cold sender, templated outreach, a deprioritized contact). The decline follows the decline register in `_shared/references/voice.md`. Present both flat and let the user pick one; the queue moves on either way. A decline is `review` tier forever — full text, an explicit go-ahead, never one-tapped and never swept into a batch approval — and it carries `action_type: "decline"` both when it is OFFERED (the `action_links.py` call that records it and returns no link, by design) and when it is logged.
   - **Email housekeeping** (no reply needed) → archive or label via the google-workspace tools.
   - **Skip / snooze** → leave it; it stays in the queue.
   - Honor the approval tiers (`_shared/references/approval-tiers.md`) before sending or archiving anything.
4. **Record handled** — collect `{identifier, channel}` for every thread the user replied to or dismissed, plus the replied email `threadId`s, write them as `{"today": "<YYYY-MM-DD>", "signals": {"handled": [...], "replied_thread_ids": [...]}}` to `/tmp/sotto_triage_cont.json`, then `execute_code` → `python3 "$HOME/.hermes/skills/sotto/morning-brief/scripts/continuity_resolve.py" --resolve-only /tmp/sotto_triage_cont.json` so those loops CLOSE and don't re-surface in the next brief. (`--resolve-only`: this pass closes, it never records new items.)
5. **Wrap** — one short line: "Cleared N (X email, Y texts). M left — say *continue triage* to keep going."

## Notes
- **Group chats: never a deep link.** `triage_queue.py` emits group threads with `is_group_chat: true` and `identifier: ""` — there is no per-group address to link to (same rule as the brief, which never emits a link for a group). Their only actions are **reply in the app yourself** or **mark-handled**; do NOT route a group item to `sotto-draft-reply`'s link builder (an empty identifier builds a broken link). Group thread lines may carry per-sender attribution ("Name: message") — quote a message only as the **named sender's**, never attribute it to the group or another member.
- **Local has no archive.** You can't archive/label a text — for iMessage/WhatsApp the only triage actions are **reply** or **mark-handled** (continuity). Archive/label is **email only**.
- Keep it fast and skimmable — triage is a clearing pass, not a brief. No prose, no calendar.
- Local data comes from the Bridge; if it's unreachable, triage **email only** and say so in one line.
- Deliver as **Sotto**, never "Hermes Agent".
