...
def _load_module(rel_path: str, name: str):
    ...
    sys.modules[name] = mod                        # so cross-module imports by bare name resolve
    spec.loader.exec_module(mod)
    return mod


from gemini import call_gemini as gemini_call  # noqa: E402  (the LLM call's owner)
...
def _score_live(name: str) -> dict:
    ...
    Returns {score, model}. Reuses gemini.call_gemini + compose_brief.run_critic — no reinvented scorer."""
    with open(os.path.join(FIX_DIR, name + ".json"), encoding="utf-8") as f:
        fixture = json.load(f)
    base = _base_datetime()
    fx = resolve_tokens(fixture, base)
...
def main() -> int:
    ...
    ap.add_argument("--golden", nargs=argparse.REMAINDER, metavar="ARGS",
                    help="replay the Golden Corpus instead of the fixtures — the owner's real, "
                         "pseudonymized history scored against his labels. EVERYTHING after --golden "
                         "goes to evals/run_golden.py (e.g. `--golden --live --sample 5`), so its "
                         "flags are defined in exactly one place.")
    ...
    if args.golden is not None:
        ...
        sys.argv = [sys.argv[0]] + list(args.golden)
        return rg.main()
    if args.live:
