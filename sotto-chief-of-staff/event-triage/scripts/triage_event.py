...
{"class":"urgent|actionable|ambient|ignore","why"}. urgent|actionable → agent; ambient → queue;
ignore → drop. ANY Tier-1 error → queue (fail toward silence, never toward noise).
...
sotto_log.bounded_append). digest_check.py consumes it; brief composers may later.
...
def _append_queue(verdict_class: str, sender: str, event: dict) -> None:
    """One JSONL line per queued event. Bounded (rotate-keeping-tail) so the file can't grow forever
    on the /data volume. Best-effort — a failed append must never fail the triage."""
    try:
        ...
        bounded_append(_queue_path(), line, QUEUE_MAX_BYTES, QUEUE_KEEP_LINES)
    except Exception:  # noqa: BLE001
        pass
...
def _classify_tier1(e: dict, one_liner: str) -> tuple[str, str, str]:
    ...
    prompt = (
        ...
        'Respond with STRICT JSON only: {"class":"urgent|actionable|ambient|ignore","why":"<one short sentence>"}\n'
        "Definitions:\n"
        ...
        "- ambient: FYI, social chatter, scheduling noise — batch it into a digest\n"
