...
LOCAL date, so it rolls over at local midnight with no cleanup job. EVERY agent verdict spends one —
fresh Tier-0/Tier-1 verdicts AND valve promotions — and beyond SOTTO_NUDGE_BUDGET (default 4/day)
further agent verdicts demote to queue with class "budget" (held_class preserved, exactly like
cooldown/quiet/stale, so the digest ranks them as deferred-actionable). BUDGET_EXEMPT_CLASSES are
...
channel within SOTTO_ESCALATION_WINDOW_MIN (default 45). Identity matching is deliberately
conservative — an exact resolved-name match or an exact normalized-identifier match (the same
last-10-digit/lowercase-email rule the graph uses); nothing fuzzy, ever. The join fires only when
...
(calcache.py) detects meetings that ENDED since the last tick — ≥ SOTTO_TAP_GRACE_MIN ago, with at
...
SOTTO_VALVE_MAX_AGE_MIN (default 4h), respecting the per-thread cooldown and an hourly budget
(SOTTO_VALVE_MAX_PER_HOUR, default 2). "meeting_hold" entries are exempt from the max-age check —
...
Env: SOTTO_DATA, SOTTO_TIMEZONE, SOTTO_QUIET_START/END (default 21/7), SOTTO_TRIAGE_MODEL,
     ...
     default 4; 0 = nudge nothing but the exempt classes), SOTTO_VIP_PRIORITY (default 10),
     ...
     never too old to deliver), SOTTO_CALENDAR_REFRESH_SECS (the receiver's calendar-cache
     cadence, default 900; the in-meeting hold's staleness bound is 2× it),
     SOTTO_ESCALATION_WINDOW_MIN (the cross-channel join window, default 45).
"""
...
def _budget_spend(day: str, n: int = 1) -> None:
    ...
    a free nudge). The write itself stays tmp + os.replace. Best-effort."""
    if n <= 0:
        return
    try:
        with _locked(_budget_path()):
            cur = _budget_spent(day)
            os.makedirs(_events_dir(), exist_ok=True)
            tmp = _budget_path() + ".tmp"
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump({"date": day, "count": cur + n}, f)
...
def _escalation_window_min() -> int:
    return max(1, _int_env("SOTTO_ESCALATION_WINDOW_MIN", ESCALATION_WINDOW_MIN_DEFAULT))
...
def _ledger_rows(now_utc: datetime, window_min: int) -> list:
    ...
    for line in _tail_lines(_queue_path(), ESCALATION_SCAN_LINES):
        ...
        if age is None or age > window_min:
            continue
        channel = _s(ev.get("source"))
        if channel == MEETING_END_SOURCE:
            continue  # the tap is Sotto's own synthetic event — a meeting ending is not the person
                      # reaching you, so it is never evidence for the escalation join
        rows.append({"channel": channel, "name": _s(entry.get("sender")),
                     "idents": _event_identifiers(ev), "age": age,
    ...
    for line in _tail_lines(_surfaced_path(), ESCALATION_SCAN_LINES):
        ...
        if channel == MEETING_END_SOURCE:
            continue  # same rule as the queue side: a surfaced tap is not a contact from the person
        ...
        rows.append({"channel": channel, "name": sender,
                     ...
                     "age": age, "cls": _s(entry.get("class"))})
    return rows
...
def triage(payload: dict, now_local=None, now_utc=None) -> dict:
    ...
    for e in events:
        ...
        if verdict == "agent":
            ...
            # would swallow it.
            elif cls not in COOLDOWN_EXEMPT_CLASSES and not _cooldown_ok(key, now_ts):
                held_class = cls
            ...
            # cooldown either — it may still promote via the valve tomorrow).
            elif cls not in BUDGET_EXEMPT_CLASSES and _budget_left(day) <= 0:
                held_class = cls
                ...
                if cls not in BUDGET_EXEMPT_CLASSES:
                    _budget_spend(day, 1)
        if verdict == "queue":
            queued = True
...
def release_valve(now_local=None, now_utc=None, now_ts=None) -> dict:
    ...
    does so the receiver stages + spawns identically. Eligibility, in order:
      ...
      KNOWN sender (the resolved name triage wrote) → younger than SOTTO_VALVE_MAX_AGE_MIN (which
    ...
    Budget: SOTTO_VALVE_MAX_PER_HOUR (default 2) across ticks, AND the shared cross-thread daily
    budget (SOTTO_NUDGE_BUDGET) — a promotion is a nudge, so it spends the same allowance a fresh
    agent verdict does and can never push the day past its cap. SOTTO_VALVE=0 disables entirely.
    ...
    with _locked(_queue_path()):
        ...
        daily_left = _budget_left(day)      # a promotion IS a nudge — it spends the day's allowance
        promoted, promoted_idx, spent, budget_blocked = [], set(), 0, False
        for i, line in enumerate(lines):     # oldest first — FIFO fairness for the longest-held ask
            ...
            exempt = held in BUDGET_EXEMPT_CLASSES
            if not exempt and daily_left <= 0:
            ...
            # An ask Sotto itself held is never too old to deliver: a meeting_hold entry aged past
            # the window only because the MEETING ran long (a 3h block outlasts the 4h default when
            # the ask arrived before it started), and expiring it silently is exactly the failure
            # the valve exists to fix. Every other held class still expires; the ≤2/hr valve cap
            # and the daily budget still bound the volume.
            if age > max_age and cls != "meeting_hold":
                continue
            key = _thread_key(ev)
            if held not in COOLDOWN_EXEMPT_CLASSES and not _cooldown_ok(key, now_ts):
                continue      # same exemption as triage(): a held escalation is not cooled down away
            _stamp_cooldown(key, now_ts)
    ...
    _budget_spend(day, spent)            # exempt promotions ride free, like their fresh verdicts
    bundle = {
        "generated_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "catchup": False,
        "promoted": True,
