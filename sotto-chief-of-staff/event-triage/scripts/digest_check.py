...
What counts toward the threshold (Sprint 0 §2a — the gate measures SIGNAL, not noise):
  ...
  - classes "quiet"/"cooldown"/"stale" from a KNOWN sender (agent-worthy verdicts that were
    deterministically deferred — the closest thing the queue has to "actionable").
...
# deterministically-deferred agent verdicts (quiet hours / cooldown / stale-catchup).
COUNT_CLASSES = frozenset({"ambient", "quiet", "cooldown", "stale"})
# Of those, the deferred-agent ones sort first in items — they were judged interrupt-worthy once.
ACTIONABLE_CLASSES = frozenset({"quiet", "cooldown", "stale"})
