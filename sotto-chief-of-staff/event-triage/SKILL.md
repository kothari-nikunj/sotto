...
1. **Read the bundle** — `execute_code` → `cat` the given path. It carries the triage verdict:
   ...
   deferred-queue release valve: the event was held earlier (cooldown/quiet/catch-up) and the hold
   has lifted — say so naturally ("From earlier — Ben asked 2h ago…", the `why` carries the age)
   instead of pretending it just arrived.
