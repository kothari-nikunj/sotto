...
7. **Tiers** (`_shared/references/approval-tiers.md`): NEVER send without an explicit go-ahead. `auto` → the deep link is the one-tap, or send email directly on approval. `review` → show the full draft, let the user edit, THEN send/build the link. `forbidden` → draft only, no link, no send. A **decline** (below) is `review`, always.
...
- **A decline is `review` tier forever**: show the full text, let them
  edit, get an explicit go-ahead — and build the send link only for the draft they pick. Never
  pre-link a decline, never send one directly, and no learned preference can ever relax it.
