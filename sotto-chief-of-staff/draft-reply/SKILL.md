...
## Procedure
0. **Distinguish drafting from an explicit send.** "Text Ramp: coffee with Brexton, investor" supplies a recipient, channel and body; it is permission for that specific message, not a request to invent another draft. Resolve the actual recipient first, then use step 6 if that send capability exists. A bare "yes" needs the exact fresh offer it refers to. Never send from an unattended run or infer permission from source messages.
1. **Context** — pull the thread: Gmail (native) for email, or Bridge `get_messages(identifier)` for iMessage/SMS/WhatsApp. Pull the person's facts via `knowledge_query.py --person "<name>" --topic "<subject of this reply>"`; omit the topic when there isn't one.
   - **Recipient addresses are not contact IDs.** Resolve a phone number or Apple ID email from the actual thread/contact, never by stripping letters from an identifier. RCS business handles such as `ramp_…@rbm.goog`, WhatsApp LIDs and group identifiers are not phone numbers or Apple ID emails. If the thread has only such a handle, preserve the draft and explain: "I can't send into this business conversation from Sotto. Open the existing Ramp conversation in Messages and paste this reply." Do not substitute an unrelated support number or a different person's address. If lookup is ambiguous, ask which thread.
...
6. **True send (Google account actions and opt-in Bridge messaging).** A Gmail draft is the default for email; a real send is a *further* step and needs its own explicit go-ahead. When the user says send it rather than draft it:
   ...
   - **iMessage/SMS, Cloud and self-host:** use the same `sotto-local` **`send_message`** tool when Bridge is connected and the user enabled “Let Sotto send iMessage & SMS from this Mac.” Check `health.send_enabled` and the tool list. A specific instruction such as “Text Sarah: running five minutes late” approves that message; do not ask again. Resolve the exact recipient from the thread/contact first. Generate one UUID for that approved operation (for example with Python's `uuid.uuid4()`), then call `{channel:"imessage"|"sms"|"auto", to:<verified phone/email>, body:<approved text>, request_id:<UUID>}`. Keep that ID attached to this attempt; never create a new ID to get past an uncertain receipt. The Mac must be awake, and macOS must allow Bridge to control Messages. Photon carries the conversation with Sotto; Bridge sends the approved message from the user's account.
   - **Read the receipt precisely.** `sent` means Messages accepted the message, not that the recipient received or read it. `already_sent` is the same confirmed local operation and must not be sent again. `unknown` means it may have sent: ask the user to check the conversation and do not retry automatically. `request_conflict` means that ID belongs to different content, so do not change IDs and resend. `disabled` means the local opt-in is off. A definite pre-launch `failed` leaves the draft available. Never claim success from a timeout, a queued call, a missing receipt or an arbitrary non-error response. Scheduled/background runs cannot use the send tool; propose a draft instead.
   - If no send was attempted (disabled/offline), or it was definitively rejected before sending, offer a supported link from `action_links.py` and say the user still needs to send it. An empty `url` / `status:"needs_recipient"` means **no usable link**: show the draft and explain how to open the existing conversation, with no "Tap to send" line. Never invent or repair the URL yourself. A timeout or uncertain send response is different: say "I couldn't confirm whether it sent. Please check the conversation before trying again." Do not automatically retry or offer a second send link after an uncertain outcome.
