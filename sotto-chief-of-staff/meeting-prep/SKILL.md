...
1. **Gather** — Calendar deterministically, the rest as usual:
   ...
   - **`read_local`** → `/tmp/sotto_local.json`, **`knowledge_query.py --calendar /tmp/sotto_cal.json --local /tmp/sotto_local.json`** → `/tmp/sotto_know.json` (run after the calendar gather — the flag packs everyone on the calendar ahead even if their graph file hasn't been touched in weeks).
