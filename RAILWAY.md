...
| `SOTTO_CRON_DELIVER` | where cron briefs are delivered — defaults to `whatsapp` (the WhatsApp home channel). Set to `local`, `telegram`, etc. to override. Also the delivery-channel gate for nudges: the "WhatsApp must be linked" check applies only while this is `whatsapp`. Personal `user-` routines deliver to the same target. | optional |
