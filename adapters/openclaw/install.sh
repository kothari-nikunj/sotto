...
echo "  • Scheduler: create the fallback crons (the Bridge push fires the real brief). A local install"
echo "    may run no trigger receiver, so the two \"runner\": \"receiver\" brief jobs are printed here"
echo "    too; where the receiver DOES run it schedules them itself and the deliver-once gate keeps"
echo "    the day to one brief. Prompt-based —"
