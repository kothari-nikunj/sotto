...
echo "      $OPENCLAW_BIN cron add \"30 12 * * *\"  \"Run the sotto-event skill in digest mode\" --name sotto-midday-digest      --tz <zone> --announce --channel whatsapp --to <your-number>   # adaptive midday catch-up (delivers only on a heavy day; skip if SOTTO_DIGEST=0)"
echo "    (No sotto-followup cron: post-meeting follow-ups run inside the evening brief — the"
echo "     sotto-followup skill stays installed for on-demand use. Same schedule as the Hermes adapter.)"
