...
- **Scheduler** → OpenClaw cron jobs are **prompt-based** (there is **no `--skill` flag**); `--tz`
  ...
  ```
  (Same shape for `sotto-evening-brief` at `30 17 * * *`, `sotto-relationship-pulse` Mon 9:00, the
  optional `sotto-proactive` at `*/15`, and the optional `sotto-midday-digest` at `30 12 * * *` —
  "Run the sotto-event skill in digest mode", an adaptive catch-up that stays silent on quiet days
  (skip it if you set `SOTTO_DIGEST=0`). That's the same five-cron schedule the Hermes installer
  creates — no standalone follow-up cron: post-meeting follow-ups run inside the evening brief, and
  the `sotto-followup` skill stays available on demand. The Bridge push fires the real brief; cron
  is the fallback — SPEC §4.1.)
