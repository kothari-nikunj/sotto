...
def main() -> int:
    ...
    # The PTY is read in fixed-size chunks, and the QR is drawn with 3-byte UTF-8 block characters —
    # so a chunk boundary lands mid-character often enough to matter. Decoding each chunk on its own
    # turned those bytes into replacement characters, punching a hole in the QR that no phone can
    # read. An incremental decoder carries the partial character across the boundary instead.
