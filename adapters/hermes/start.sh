...
# briefs run headlessly, so nothing passes --user-email — this var is the only source. There is no
# cheap on-disk copy of the Google-authorized address (the token lives inside the google-workspace
# skill's own store, and asking Google for it would be a network call at boot), so when it's unset
# we say so once rather than guess: the calendar cache's own inference (calcache._infer_self_email,
# ≥2 peopled events agreeing) still covers the tap, and the brief simply researches a few people it
# needn't have.
if [ -z "${SOTTO_USER_EMAIL:-}" ]; then
  echo "[sotto] note: SOTTO_USER_EMAIL unset — set it in Railway to your own address so attendee"
  echo "[sotto]       research and post-meeting taps can exclude you (see RAILWAY.md)."
fi
...
MARKERS = ("sotto-morning-brief", "sotto-evening-brief", "sotto-relationship-pulse", "sotto-proactive",
           "sotto-followup", "sotto-midday-digest",
           "Run my morning brief", "Run my evening brief", "Run my relationship pulse",
           "Run my proactive check", "Run my followup", "Run my midday digest")
try:
...
# Post-meeting follow-up cron RETIRED (Sprint 0): the standalone 16:45 followup was near-identical to
# the 17:30 evening brief 45 minutes later — the EA pattern is ONE end-of-day report, so its content
# now lands inside the evening brief. The `sotto-followup` skill remains for on-demand runs and the
# coming post-meeting trigger. "sotto-followup"/"Run my followup" stay in MARKERS above so existing
# registrations from earlier deploys are removed on every boot.
# Midday catch-up digest (Proactiveness Phase 2): 12:30 local, runs the sotto-event skill in DIGEST
# mode. Adaptive — digest_check.py delivers only when the ambient event queue since the last
# digest/brief is heavy (≥ SOTTO_DIGEST_MIN, default 8); a quiet day stays silent. Never auto-sends.
# Default on; SOTTO_DIGEST=0 disables it (the dedup above still removes a stale one).
if [ "${SOTTO_DIGEST:-1}" = "1" ]; then
  case "$crons" in *sotto-midday-digest*|*"Run my midday digest"*) ;; *) hermes cron create "30 12 * * *" "Run my midday digest" --skill sotto-event --name sotto-midday-digest --deliver "$SOTTO_CRON_DELIVER" 2>&1 | sed 's/^/[sotto] cron-create digest: /' || true ;; esac
fi
