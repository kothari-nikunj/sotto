...
# Post-meeting follow-up cron RETIRED (Sprint 0): the followup content now runs inside the 17:30
# evening brief (one end-of-day report). The sotto-followup skill stays installed for on-demand use;
# the cloud boot (start.sh) removes any 16:45 cron left over from earlier installs.
# Midday catch-up digest (12:30 local; runs sotto-event in DIGEST mode — digest_check.py delivers only
# on a heavy day, else silent; never auto-sends). Default on; SOTTO_DIGEST=0 to skip.
if [ "${SOTTO_DIGEST:-1}" = "1" ]; then
  case "$crons" in *sotto-midday-digest*|*"Run my midday digest"*) note "cron sotto-midday-digest exists — skip" ;; *) run hermes cron create "30 12 * * *" "Run my midday digest" --skill sotto-event --name sotto-midday-digest --deliver "$SOTTO_CRON_DELIVER" ;; esac
fi
