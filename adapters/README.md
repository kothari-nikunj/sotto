...
**`"runner": "receiver"` — the briefs, the watcher, the digest, and the weekly pulse.** All five
system rows carry it, so no
...
re-register. The weekly pulse (`0 9 * * 1`) follows the same receiver path.
