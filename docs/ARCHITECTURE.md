...
| **learn** | [`_shared/scripts/learn_step.py`](../sotto-chief-of-staff/_shared/scripts/learn_step.py) — ONE command that runs `knowledge_update.py`, `continuity_resolve.py --merge-only`, `learn_preferences.py`, `style_extract.py`, `granola_graph.py` and `prewarm_graph.py --sync-contacts` in order | What the brief found is written back into memory, so tomorrow starts from today — and the step leaves a receipt (`briefs/<date>.<kind>.learned.json`) naming what ran, which the send seam checks on the channel's ack: a brief that delivered without learning is said out loud. Six separate instructions used to stand where this command is, with nothing verifying any of them ran (Sep 2026). |
...
| Sotto cron (`receiver.start_cron_thread`) | `receiver.CRON_TICK_SECS` = 60s | Fires every `crons.json` job marked `"runner": "receiver"` — the morning and evening briefs, the proactive watcher and the midday digest — when its slot arrives, down the same spawn → outbox path every other lane takes (so a run that ends in the `NO_NUDGES` sentinel is an `empty` receipt, never a message — Hermes' own scheduler delivered the literal token on Sep 4, 2026). One fire per job per slot in memory (`_cron_slot`: the local day, or the day plus the interval boundary) — and the durable `.delivered` marker is consulted first, so a restart inside the window never recomposes a day that already went out; a brief run that DIES is re-fired once the same day (`_retry_failed_brief`), and the wake-push's "the cron is composing, fold instead" decision reads `_RUNS_INFLIGHT`, so it never stands down for a cron run that is already dead. A timezone change clears the in-memory fired stamps (`set_timezone`) — the zone is re-read every tick, so nothing needs re-registration. Two schedule shapes: fixed daily `M H * * *` (one slot per local day) and `*/N * * * *` intervals (one slot per N-minute boundary, caught up inside the same `BRIEF_CRON_WINDOW_MIN` window); a nudge lane (watcher, digest) is held by the channel-linked gate like the wake-push and the valve, a brief never is. **The same tick also runs the nightly housekeeping** at `retention.SWEEP_LOCAL` (3:30 AM local, one fired-today map): the retention sweep (one log line every day, including a zero), and the daily chat-session archive (`sessions.archive_all`, the same call the boot path makes — silent `hermes sessions archive`, so a chat session lasts a day or a deploy, whichever comes first, and interactive replies have no week-old transcript to copy from) on its own daemon thread, so an external CLI can never hold the thread that fires the briefs |
...
| `briefs/<date>.<kind>.learned.json` | skills (`learn_step.py` — the Learn step's receipt: which of the six memory writers ran, skipped or failed) | receiver (`_on_delivered` — a brief the channel ACKED with no recent receipt is logged as a brief that did not learn) |
...
| `proactive/mute_offers.json` | skills (`compose_brief._stamp_mute_offer` — the evening brief's "stop bringing Bob up?" line) | skills (`compose_brief._pick_mute_candidate`) — one date per person asked, so the question about one person comes at most once a month |
...
**Who runs a job is a field, not a second file.** A row marked `"runner": "receiver"` — the
morning and evening briefs, the proactive watcher and the midday digest — is never registered with
Hermes; `receiver._cron_tick` fires it on the 60s heartbeat instead, so the run is composed and
delivered down the receiver's one lane (spawn → outbox → silence seam → deliver-once gate) and
inherits its retries, its receipts, and the `NO_NUDGES` sentinel handling a Hermes-delivered run
never had (Sep 4, 2026: the literal token reached the user's Telegram at 7:02 from Hermes' clock).
The tick reads two shapes — fixed daily `M H * * *` and `*/N * * * *` intervals — and the row's own
`prompt` rides into the spawn quoted as the job's name, which is how the two-mode `sotto-event`
skill knows a scheduled run is the digest. Hermes' scheduler still runs the weekly pulse (a weekday
schedule the tick does not read). The reconciler still lists the receiver-run rows among its REMOVAL
markers, which is how an already-deployed box sheds its old Hermes crons on the next boot — the
migration needs no manual step.
