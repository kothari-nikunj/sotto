...
Gemini-shaped response, and gets its web research from Gemini's own Search Grounding. A key from any
other provider does nothing for briefs.
...
Sotto code changes**. You still need the Gemini key for the briefs themselves.
...
scope is one file, and the honest blocker isn't context or price, it's grounded search (below).
...
| 4 | `_shared/scripts/research_attendees.py` | **Attendee research** — Gemini + `google_search` grounding, 5 attendees/batch, cap 25 | ≤5 batches per brief (+ deep/focus passes) | 2,360 chars/batch |
| 5 | `_shared/scripts/web_research.py` | Ad-hoc grounded lookup (schedule skill, venue checks) | on demand | query only (~10²  chars) |
...
| **Search story** | **Native `google_search` grounding — this is what sites 4–5 use** | No native web search in the pipeline's call shape; Anthropic's server-side web-search tool exists but is not wired here | No native grounding in our call shape | None | None |
...
| **THE GAP — research (sites 4–5)** | — (works) | **No substitute in-tree.** `research_attendees.py` / `web_research.py` are hard-wired to `tools:[{google_search:{}}]`; there is no Brave/SerpAPI path anywhere in this repo | same | same | same |
...
and they neither know nor care what the chat model is. `start.sh` currently pins the chat model to
Gemini so one key covers everything; overriding that is a config change, not a code change.
...
### (c) What must be REQUIRED when Gemini is absent

> **Rule: no Gemini key ⇒ no grounded research, and the brief must say so rather than invent.**

There is no second search provider in this repo — no Brave, no SerpAPI, no execution-policy fallback.
Sites 4 and 5 are Gemini Search Grounding and nothing else. So a non-Gemini deployment must either:

1. **carry a Gemini key anyway, for research only** (cheapest and most honest: research is ~5,900
   input tok/day, cents), or
2. **ship a search provider alongside the base-URL client** — a `SOTTO_SEARCH_API_KEY` that becomes
   *required* whenever the LLM provider is not Gemini, wired into `web_research.py` and
   `research_attendees.py` behind the same `research()` signature they already expose.

Until (2) exists, "runs without Gemini" means a brief with no attendee background — which the
pipeline degrades to safely (research failures return empty and the brief omits rather than invents),
but it is a materially thinner product and must be documented as such, not silently shipped.
...
4. Research stays on Gemini until a search provider lands — per rule (c).
